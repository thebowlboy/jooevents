<script lang="ts">
	...
	// The picker is re-triggerable — it loads with the shell and refreshes on
	// every open — so it needs both ordering guarantees: a reopen while a request
	// is still open must not stack a second one, and a slow earlier answer must
	// never replace the fresher list already on screen. A rejected list becomes a
	// stated failure inside the panel; it used to become a skeleton that never
	// resolved, because the read had no rejection path at all.
	let eventsState = $state<LiveReadState<WorkspaceEventOption[]>>({ kind: 'resolving' });
	const eventsRead = new LiveRead<WorkspaceEventOption[]>({
		read: async () => {
			const collection = port.events;
			// Unreachable from the UI: the picker only mounts where the capability
			// exists. Stated rather than assumed, so a future caller gets an answer
			// instead of a promise that never settles.
			if (!collection) throw new Error('This workspace does not serve an event list.');
			return [...(await collection.list())];
		},
		fallback: 'The event list could not be loaded.',
		onChange: (state) => (eventsState = state)
	});
	const events = $derived(eventsState.kind === 'resolved' ? eventsState.value : null);

	function loadEvents() {
		if (!port.events) return;
		void eventsRead.read();
	}
		...
		{:else if summary.event}
			...
			{#if eventCollection}
				<div class="side__switch">
					...
					{/snippet}
					{#snippet children()}
						<div class="evswitch" aria-busy={eventsState.kind === 'resolving' || undefined}>
							{#if eventsState.kind === 'unavailable'}
								<!-- The list answered, and the answer was no. A skeleton here
								     would keep promising options that are not coming. -->
								<p class="evswitch__refusal" role="alert">{eventsState.message}</p>
								{#if eventsState.retryable}
									<div class="evswitch__retry">
										<button
											type="button"
											class="ui-button ui-button--secondary ui-button--sm"
											onclick={() => void eventsRead.refresh()}>Try again</button>
									</div>
								{/if}
							{:else if !events}
...
<style>
	...
	/* The retry sits with the failure it answers, so no rule separates them. */
	.evswitch__retry {
		display: grid;
		justify-items: start;
