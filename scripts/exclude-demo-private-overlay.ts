...
export interface GeneratedReviewOverlayExclusionResult {
...
function inspectGeneratedTree(root: string, directory: string): number {
  ...
  for (const name of readdirSync(directory)) {
    ...
    if (child !== root && !child.startsWith(`${root}${sep}`)) {
      throw new TypeError('generated_review_overlay_child_escaped_target');
    }
    const stat = lstatSync(child);
    if (stat.isSymbolicLink()) {
      throw new TypeError('generated_review_overlay_contains_symbolic_link');
    }
    if (stat.isDirectory()) {
      fileCount += inspectGeneratedTree(root, child);
      continue;
    }
    if (!stat.isFile()) throw new TypeError('generated_review_overlay_contains_special_file');
    fileCount += 1;
  }
  return fileCount;
}
 ...
 */
export function excludeGeneratedReviewOverlay(
  buildDirectory: string
): GeneratedReviewOverlayExclusionResult {
  const buildRoot = resolve(buildDirectory);
  assertOrdinaryDirectory(buildRoot, 'generated_review_build_root');
  const target = resolve(buildRoot, 'reviews');
  if (relative(buildRoot, target) !== 'reviews') {
    throw new TypeError('generated_review_overlay_target_invalid');
  }
  if (!existsSync(target)) return Object.freeze({ removed: false, fileCount: 0 });

  assertOrdinaryDirectory(target, 'generated_review_overlay_target');
  const fileCount = inspectGeneratedTree(target, target);
  rmSync(target, { recursive: true });
  if (existsSync(target)) throw new TypeError('generated_review_overlay_exclusion_failed');
  return Object.freeze({ removed: true, fileCount });
}

/** Retained export name for the demo build command. */
export const excludeDemoPrivateOverlay = excludeGeneratedReviewOverlay;

if (import.meta.main) {
  const buildRoot = resolve(import.meta.dir, '../apps/web/build');
  const result = excludeGeneratedReviewOverlay(buildRoot);
