...
ALTER TABLE submission_triage_heads RENAME TO submission_triage_heads_e2_0001;
...
SELECT
  ...
  submission_id,
  head_version,
  CASE state WHEN 'discarded_recoverable' THEN 'spam' ELSE state END,
  updated_at_ms,
  CASE state
    WHEN 'discarded_recoverable' THEN (
      SELECT transformed.head_json
        FROM temp.e2_0002_submission_triage_spam_rows AS transformed
       WHERE transformed.workspace_id = submission_triage_heads_e2_0001.workspace_id
         AND transformed.event_id = submission_triage_heads_e2_0001.event_id
         AND transformed.submission_id = submission_triage_heads_e2_0001.submission_id
    )
    ELSE head_json
  END,
  CASE state
  ...
  END
FROM submission_triage_heads_e2_0001;

DROP TABLE submission_triage_heads_e2_0001;
DROP TABLE temp.e2_0002_submission_triage_spam_rows;
