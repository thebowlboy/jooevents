...
/** Disposable SQLite evaluation schema; this is not a retained migration. */
export const REVIEW_TRIAL_SQL = `
...
export type SQLiteReviewTrialErrorCode =
...
export class SQLiteReviewTrialError extends Error {
  constructor(readonly code: SQLiteReviewTrialErrorCode, cause?: unknown) {
    super(code, cause === undefined ? undefined : { cause });
    this.name = 'SQLiteReviewTrialError';
  }
}
...
export function installReviewTrialSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new SQLiteReviewTrialError('transaction_required');
  sqlite.exec('PRAGMA foreign_keys = ON;');
  sqlite.exec(REVIEW_TRIAL_SQL);
}

export class SQLiteReviewTrialRepository implements ReviewTransactionRepository {
  ...
  readCatalog(scopeInput: ReviewScopeDto): ReviewCatalogDto | undefined {
    ...
    if (rows.length > 1) throw new SQLiteReviewTrialError('data_corrupt');
    const rounds = this.readRoundRows(scope).map((row) => this.roundFromRow(row));
    if (!rows[0]) {
      if (rounds.length > 0) throw new SQLiteReviewTrialError('data_corrupt');
  ...
  applyCatalog(input: { readonly before: ReviewCatalogDto; readonly after: ReviewCatalogDto }): void {
    ...
    if (before.scope.workspaceId !== after.scope.workspaceId
      ...
      throw new SQLiteReviewTrialError('stale_catalog');
    }
    if (before.version === 1) {
      try {
        ...
        if (error instanceof SQLiteReviewTrialError) throw error;
        throw new SQLiteReviewTrialError('stale_catalog', error);
      }
      return;
    }
  ...
  insertRound(round: ReviewRoundDto): void {
    ...
    try {
      ...
      if (error instanceof SQLiteReviewTrialError) throw error;
      throw new SQLiteReviewTrialError('identity_collision', error);
    }
  }
  ...
  updateRound(input: { readonly before: ReviewRoundDto; readonly after: ReviewRoundDto }): void {
    ...
    if (before.id !== after.id
        ...
        || before.scope.eventId !== after.scope.eventId
        || after.version !== before.version + 1) {
      throw new SQLiteReviewTrialError('stale_round');
    }
  ...
  updateAssignment(input: { readonly before: ReviewAssignmentDto; readonly after: ReviewAssignmentDto }): void {
    ...
    if (before.id !== after.id
        ...
        || before.scope.eventId !== after.scope.eventId
        || after.version !== before.version + 1) {
      throw new SQLiteReviewTrialError('stale_assignment');
    }
  ...
  saveDraft(input: { readonly expectedVersion: number | null; readonly draft: ReviewDraftDto }): void {
    ...
    if (draft.version !== (input.expectedVersion ?? 0) + 1) {
      throw new SQLiteReviewTrialError('stale_draft');
    }
    if (input.expectedVersion === null) {
      try {
        ...
        if (error instanceof SQLiteReviewTrialError) throw error;
        throw new SQLiteReviewTrialError('stale_draft', error);
      }
      return;
    }
  ...
  insertFirstReview(input: { readonly head: ReviewHeadDto; readonly revision: ReviewRevisionDto }): void {
    ...
    if (head.version !== 1
      ...
      throw new SQLiteReviewTrialError('identity_collision');
    }
    try {
  ...
  appendReviewRevision(input: {
    ...
    if (before.assignmentId !== after.assignmentId
      ...
      throw new SQLiteReviewTrialError('stale_review');
    }
    try {
      this.insertRevision(revision);
      ...
      if (error instanceof SQLiteReviewTrialError) throw error;
      throw new SQLiteReviewTrialError('stale_review', error);
    }
  }
  ...
  private requireTransaction(): void {
    if (!this.sqlite.inTransaction) throw new SQLiteReviewTrialError('transaction_required');
  }
}
...
function instant(milliseconds: number): string {
  ...
  if (Date.parse(value) !== milliseconds) throw new SQLiteReviewTrialError('data_corrupt');
  return value;
}

function changedExactlyOnce(
  result: { readonly changes: number },
  code: SQLiteReviewTrialErrorCode
): void {
  if (result.changes !== 1) throw new SQLiteReviewTrialError(code);
}
...
function guarded<Value>(read: () => Value): Value {
  try {
    ...
    if (error instanceof SQLiteReviewTrialError) throw error;
    throw new SQLiteReviewTrialError('data_corrupt', error);
  }
}
