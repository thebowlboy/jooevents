...
import {
  ...
  ENGAGEMENT_CHANGESET_COMMIT_FACT_KIND,
...
export const SQLITE_ENGAGEMENT_CHANGESET_EFFECT_SQL = `
CREATE TABLE engagement_changeset_receipt_links (
...
CREATE TABLE engagement_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'engagement_changed'),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES engagement_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE engagement_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES engagement_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE engagement_changeset_timeline (
...
CREATE TRIGGER engagement_changeset_receipt_links_no_update
BEFORE UPDATE ON engagement_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'engagement changeset receipt links are immutable'); END;
CREATE TRIGGER engagement_changeset_receipt_links_no_delete
BEFORE DELETE ON engagement_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'engagement changeset receipt links are immutable'); END;
CREATE TRIGGER engagement_changeset_domain_facts_no_update
BEFORE UPDATE ON engagement_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'engagement changeset facts are immutable'); END;
CREATE TRIGGER engagement_changeset_domain_facts_no_delete
BEFORE DELETE ON engagement_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'engagement changeset facts are immutable'); END;
CREATE TRIGGER engagement_changeset_outbox_pointers_no_update
BEFORE UPDATE ON engagement_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'engagement changeset pointers are immutable'); END;
CREATE TRIGGER engagement_changeset_outbox_pointers_no_delete
BEFORE DELETE ON engagement_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'engagement changeset pointers are immutable'); END;
CREATE TRIGGER engagement_changeset_timeline_no_update
BEFORE UPDATE ON engagement_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'engagement changeset timeline is immutable'); END;
CREATE TRIGGER engagement_changeset_timeline_no_delete
BEFORE DELETE ON engagement_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'engagement changeset timeline is immutable'); END;
`;

export function installEngagementChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('engagement_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(SQLITE_ENGAGEMENT_CHANGESET_EFFECT_SQL)).immediate();
}

export interface SQLiteEngagementChangesetEffectIds extends ChangesetLifecycleIds {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`engagement_changeset_${label}_invalid`);
  }
  return value;
}
...
function exactSubjects(context: EffectInvocationContext, eventId: EventId): boolean {
  return context.scope.eventId === eventId
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === ENGAGEMENT_CHANGESET_OWNER_ID
    );
}
...
function operationPlan(input: {
  readonly bundle: EngagementChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): EngagementChangesetPlan | undefined {
  if (input.operation.kind !== ENGAGEMENT_CHANGESET_KIND
      || input.operation.version !== ENGAGEMENT_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as EngagementChangesetPlan | undefined;
}

function ownsEngagementChangeset(input: {
  readonly bundle: EngagementChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
/**
 * Owns the engagement response changeset transitions and the effective
 * response commit on one caller-owned SQLite handle. Every lifecycle and
 * engagement head write stays inside the Foundation effect unit of work; the
 * planning snapshot and commit transaction serve the three engagement ports
 * from the one engagement repository. Commits emit facts only; no messaging
 * effect ever originates in this transaction. Seeding never flows through
 * this owner — `invited` rows are written exclusively by the acceptance
 * commit's seed collaboration inside the decision changeset domain.
 */
export class SQLiteEngagementChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  constructor(private readonly input: {
    ...
    /** The engagement draft approval policy, supplied by the operations layer. */
    readonly approvalPolicy: CapturedChangesetApprovalPolicy;
    /** The engagement draft permission, supplied by the operations layer. */
    ...
    for (const method of ID_METHODS) {
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('engagement_changeset_id_factory_invalid');
      }
    }
    this.#ids = Object.freeze(Object.fromEntries(
      ID_METHODS.map((method) => [method, input.ids[method].bind(input.ids)])
    ) as unknown as SQLiteEngagementChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:engagement:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsEngagementChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: ENGAGEMENT_CHANGESET_OWNER_ID,
      evidenceIds: Object.freeze([
        `engagement-definition:${this.#bundle.registry.registryDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) throw new TypeError('engagement_changeset_transaction_required');
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('engagement_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') throw new TypeError('engagement_changeset_approval_not_mounted');
    ...
    if (action === undefined
      ...
      throw new TypeError('engagement_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        )) throw new TypeError('engagement_changeset_authority_mismatch');
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: ENGAGEMENT_CHANGESET_OWNER_ID
      },
      evaluatedAt
    });
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('engagement_changeset_relationship_mismatch');
    }
    this.clearTransient();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('engagement_changeset_context_substitution');
          }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('engagement_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('engagement_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('engagement_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
    ...
    if (!revision) throw new TypeError('engagement_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: ENGAGEMENT_CHANGESET_COMMIT_FACT_KIND,
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('engagement_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('engagement_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('engagement_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('engagement_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO engagement_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('engagement_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('engagement_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO engagement_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO engagement_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO engagement_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('engagement_changeset_incomplete');
      }
  ...
  private nextId(method: keyof SQLiteEngagementChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('engagement_changeset_ids_not_unique');
    this.#issuedIds.add(value);
    return value;
  }

  private repository(): SQLiteEngagementRepository {
    return new SQLiteEngagementRepository(this.input.sqlite);
  }
  ...
  private currentCommitEvidence(input: {
    readonly record: StoredChangesetRecord;
    readonly repository: SQLiteEngagementRepository;
  }) {
    const eventId = input.record.head.eventId;
    if (eventId === undefined) throw new TypeError('engagement_changeset_event_required');
    const scope: EngagementScopeDto = {
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === ENGAGEMENT_CHANGESET_OWNER_ID
      ? record
      : undefined;
  }
}

export function createSQLiteEngagementChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly approvalPolicy: CapturedChangesetApprovalPolicy;
  readonly permissionId: PermissionId;
  ...
  const adapter = new SQLiteEngagementChangesetEffectDomainAdapter(input);
  return Object.freeze({
