...
import {
  EVENT_SETTINGS_CHANGESET_KIND,
  EVENT_SETTINGS_CHANGESET_VERSION,
  assertEventSettingsOrdinaryChangesetBundle,
  assertEventOrdinaryPolicy,
  createEventSettingsOrdinaryChangesetBundle,
  eventAggregateId,
  eventSettingsReadPort,
  eventSettingsTransactionPort,
  eventSettingsValidationPort,
  workspaceEventSetDigest,
  workspaceEventSetGuardId,
  type EventSettingsChangesetPlan,
  type EventSettingsOrdinaryChangesetBundle,
...
export const EVENT_SETTINGS_CHANGESET_EFFECT_SQL = `
CREATE TABLE event_settings_changeset_receipt_links (
...
CREATE TABLE event_settings_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'event_settings_changed'),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES event_settings_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE event_settings_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES event_settings_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE event_settings_changeset_timeline (
...
CREATE TRIGGER event_settings_changeset_receipt_links_no_update
BEFORE UPDATE ON event_settings_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'event settings changeset receipt links are immutable'); END;
CREATE TRIGGER event_settings_changeset_receipt_links_no_delete
BEFORE DELETE ON event_settings_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'event settings changeset receipt links are immutable'); END;
CREATE TRIGGER event_settings_changeset_domain_facts_no_update
BEFORE UPDATE ON event_settings_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'event settings changeset facts are immutable'); END;
CREATE TRIGGER event_settings_changeset_domain_facts_no_delete
BEFORE DELETE ON event_settings_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'event settings changeset facts are immutable'); END;
CREATE TRIGGER event_settings_changeset_outbox_pointers_no_update
BEFORE UPDATE ON event_settings_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'event settings changeset pointers are immutable'); END;
CREATE TRIGGER event_settings_changeset_outbox_pointers_no_delete
BEFORE DELETE ON event_settings_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'event settings changeset pointers are immutable'); END;
CREATE TRIGGER event_settings_changeset_timeline_no_update
BEFORE UPDATE ON event_settings_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'event settings changeset timeline is immutable'); END;
CREATE TRIGGER event_settings_changeset_timeline_no_delete
BEFORE DELETE ON event_settings_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'event settings changeset timeline is immutable'); END;
`;

export function installEventSettingsChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('event_settings_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(EVENT_SETTINGS_CHANGESET_EFFECT_SQL)).immediate();
}
...
export function createSQLiteEventSettingsChangesetLifecycleStore(
  sqlite: Database
): SQLiteChangesetLifecycleStore {
  return new SQLiteChangesetLifecycleStore(sqlite, terminalReceiptSource(sqlite));
}

export interface SQLiteEventSettingsChangesetEffectIds extends ChangesetLifecycleIds {
...
export interface EventSettingsApproverAuthoritySource {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`event_settings_changeset_${label}_invalid`);
  }
  return value;
}
...
function operationPlan(input: {
...
}): EventSettingsChangesetPlan | undefined {
  if (input.operation.kind !== EVENT_SETTINGS_CHANGESET_KIND
      || input.operation.version !== EVENT_SETTINGS_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as EventSettingsChangesetPlan | undefined;
}

function ownsEventSettingsChangeset(input: {
  readonly bundle: EventSettingsOrdinaryChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
function approvalPolicy(bundle: EventSettingsOrdinaryChangesetBundle) {
  return Object.freeze({
...
export class SQLiteEventSettingsChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle: EventSettingsOrdinaryChangesetBundle;
  readonly #ids: SQLiteEventSettingsChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    readonly ids: SQLiteEventSettingsChangesetEffectIds;
    readonly approverAuthority?: EventSettingsApproverAuthoritySource;
  }) {
    this.input = Object.freeze({ ...input, workspaceId: parseWorkspaceId(input.workspaceId) });
    assertEventOrdinaryPolicy(input.policy);
    this.#bundle = createEventSettingsOrdinaryChangesetBundle({ policy: input.policy });
    assertEventSettingsOrdinaryChangesetBundle(this.#bundle);
    this.lifecycleStore = createSQLiteEventSettingsChangesetLifecycleStore(input.sqlite);
    for (const method of [
      ...
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('event_settings_changeset_id_factory_invalid');
      }
    }
    ...
    ) as unknown as SQLiteEventSettingsChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:event_settings:${eventId}:${this.#bundle.policy.definitionDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsEventSettingsChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: 'event_settings',
      evidenceIds: Object.freeze([
        `event-settings-policy:${this.#bundle.policy.definitionDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) {
      throw new TypeError('event_settings_changeset_transaction_required');
    }
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('event_settings_changeset_capability_mismatch');
    }
    ...
    if (action === undefined
      ...
      throw new TypeError('event_settings_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
      ...
      throw new TypeError('event_settings_changeset_authority_mismatch');
    }
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: 'event_settings'
      },
      evaluatedAt
    });
    if (eventRelationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('event_settings_changeset_relationship_mismatch');
    }
    this.#prepared.clear();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('event_settings_changeset_context_substitution');
          }
  ...
  private nextId(method: keyof SQLiteEventSettingsChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) {
      throw new TypeError('event_settings_changeset_ids_not_unique');
    }
    this.#issuedIds.add(value);
    return value;
  }

  private repository(): SQLiteEventSettingsRepository {
    return new SQLiteEventSettingsRepository(this.input.sqlite);
  }

  private planningSnapshot(repository: SQLiteEventSettingsRepository): ChangesetPlanningSnapshot {
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== eventSettingsReadPort) {
          throw new TypeError('event_settings_changeset_undeclared_read_port');
        }
  ...
  private commitTransaction(repository: SQLiteEventSettingsRepository): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(
        key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>
      ): Port {
        if ((key as unknown) !== eventSettingsValidationPort
            && (key as unknown) !== eventSettingsTransactionPort) {
          throw new TypeError('event_settings_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('event_settings_changeset_event_required');
    const state = input.repository.readEventSettings({
      workspaceId: input.record.head.workspaceId,
      eventId
    });
    if (!state) throw new TypeError('event_settings_changeset_domain_missing');
    const aggregateVersions = new Map<string, number>([
      [eventAggregateId(eventId), state.event.version]
    ]);
    const guardId = workspaceEventSetGuardId(state.eventSet.workspaceId);
    return Object.freeze({
      aggregateVersions,
      guardVersions: new Map<string, number>([[guardId, state.eventSet.version]]),
      guardDigests: new Map<string, string>([[guardId, workspaceEventSetDigest(state.eventSet)]])
    });
  }
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === 'event_settings'
      ? record
      : undefined;
  }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('event_settings_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('event_settings_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('event_settings_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
    ...
    if (!revision) throw new TypeError('event_settings_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: 'event_settings_changed',
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('event_settings_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('event_settings_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('event_settings_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('event_settings_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO event_settings_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('event_settings_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('event_settings_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      if (child.eventId === undefined) {
        throw new TypeError('event_settings_changeset_fact_event_required');
      }
      this.input.sqlite.query<never, [
        string, string, string, string, string, string, string, number, string
      ]>(`
        INSERT INTO event_settings_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO event_settings_changeset_outbox_pointers (
      ...
      if (child.eventId === undefined) {
        throw new TypeError('event_settings_changeset_timeline_event_required');
      }
      this.input.sqlite.query<never, [
        string, string, string, string, string, string, string, number
      ]>(`
        INSERT INTO event_settings_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('event_settings_changeset_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
...
export function createSQLiteEventSettingsChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly policy: EventOrdinaryPolicy;
  readonly eventRelationships: SQLiteOperatorEventRelationshipSource;
  ...
  const adapter = new SQLiteEventSettingsChangesetEffectDomainAdapter(input);
  return Object.freeze({
    ownerId: 'event_settings' as const,
