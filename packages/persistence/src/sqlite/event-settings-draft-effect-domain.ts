...
import {
  ...
  captureEventSettingsChangesetApprovalPolicy,
  createEventSettingsOrdinaryChangesetBundle,
  eventSettingsReadPort,
  type EventSettingsOrdinaryChangesetBundle,
  type EventSettingsReadPort,
  type EventOrdinaryPolicy
} from '@jooevents/event';
import {
  EVENT_SETTINGS_UPDATE_DRAFT_HANDLER_CAPABILITY,
  EVENT_SETTINGS_UPDATE_DRAFT_OPERATION,
  EVENT_MANAGE_ACCESS_POLICY,
  eventSettingsUpdateDraftContributionSchema,
  eventSettingsUpdateDraftDomainContributionSchema,
  eventSettingsUpdateDraftEvidenceChildSchema,
  sealEventSettingsUpdateDraftPreparation,
  type EventSettingsUpdateDraftContribution
...
export const EVENT_SETTINGS_UPDATE_DRAFT_EFFECT_SQL = `
CREATE TABLE event_settings_update_draft_receipt_links (
  ...
  operation_name TEXT NOT NULL CHECK(operation_name = 'event.settings.update.draft'),
...
CREATE TABLE event_settings_update_draft_timeline (
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES event_settings_update_draft_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TRIGGER event_settings_update_draft_receipt_links_no_update
BEFORE UPDATE ON event_settings_update_draft_receipt_links
BEGIN SELECT RAISE(ABORT, 'event create draft receipt links are immutable'); END;
CREATE TRIGGER event_settings_update_draft_receipt_links_no_delete
BEFORE DELETE ON event_settings_update_draft_receipt_links
BEGIN SELECT RAISE(ABORT, 'event create draft receipt links are immutable'); END;
CREATE TRIGGER event_settings_update_draft_timeline_no_update
BEFORE UPDATE ON event_settings_update_draft_timeline
BEGIN SELECT RAISE(ABORT, 'event create draft timeline is immutable'); END;
CREATE TRIGGER event_settings_update_draft_timeline_no_delete
BEFORE DELETE ON event_settings_update_draft_timeline
BEGIN SELECT RAISE(ABORT, 'event create draft timeline is immutable'); END;
`;

export function installEventSettingsUpdateDraftEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('event_settings_update_draft_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(EVENT_SETTINGS_UPDATE_DRAFT_EFFECT_SQL)).immediate();
}

export interface SQLiteEventSettingsUpdateDraftEffectIds {
...
type DraftSuccess = Extract<
  EventSettingsUpdateDraftContribution,
...
function applicationUuid(value: unknown, label: string): string {
  if (typeof value !== 'string' || !APPLICATION_UUID.test(value)) {
    throw new TypeError(`event_settings_update_draft_${label}_invalid`);
  }
  return value.toLowerCase();
}

function exactCapability(value: { readonly key: string; readonly version: number }): boolean {
  return value.key === EVENT_SETTINGS_UPDATE_DRAFT_HANDLER_CAPABILITY.key
    && value.version === EVENT_SETTINGS_UPDATE_DRAFT_HANDLER_CAPABILITY.version;
}
...
function collisionContribution(): EventSettingsUpdateDraftContribution {
  return eventSettingsUpdateDraftContributionSchema.parse({
    result: {
      kind: 'outcome',
      outcome: {
        class: 'conflict',
...
function eventRequiredContribution(): EventSettingsUpdateDraftContribution {
  return eventSettingsUpdateDraftContributionSchema.parse({
    result: {
      kind: 'outcome',
      outcome: {
...
function planningRefusal(
  error: EventSettingsPlanningError,
  eventId: string
): EventSettingsUpdateDraftContribution | undefined {
  ...
  return eventSettingsUpdateDraftContributionSchema.parse({
    result: {
      ...
      outcome: {
        ...
        detail: { code: error.code, action: 'update', eventId },
        detailSchemaVersion: 1
      }
    },
    domain: null,
...
/** Persists an inert selected-Event settings draft in the Foundation UoW. */
export class SQLiteEventSettingsUpdateDraftEffectDomainAdapter implements SQLiteEffectDomainAdapter {
  readonly #bundle: EventSettingsOrdinaryChangesetBundle;
  readonly #changesets: SQLiteChangesetLifecycleStore;
  readonly #ids: SQLiteEventSettingsUpdateDraftEffectIds;
  ...
  constructor(private readonly input: {
    ...
    for (const method of [
      ...
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('event_settings_update_draft_id_factory_invalid');
      }
    }
    ...
    ) as unknown as SQLiteEventSettingsUpdateDraftEffectIds);
  }

  private nextId(method: keyof SQLiteEventSettingsUpdateDraftEffectIds): string {
    const value = applicationUuid(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('event_settings_update_draft_ids_not_unique');
    this.#issuedIds.add(value);
    return value;
  }

  private readPort(): EventSettingsReadPort {
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) {
      throw new TypeError('event_settings_update_draft_transaction_required');
    }
    if (!exactCapability(capability)) {
      throw new TypeError('event_settings_update_draft_capability_mismatch');
    }
    if (context.operation.name !== EVENT_SETTINGS_UPDATE_DRAFT_OPERATION.name
        || context.operation.version !== EVENT_SETTINGS_UPDATE_DRAFT_OPERATION.version
      ...
      throw new TypeError('event_settings_update_draft_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        )) throw new TypeError('event_settings_update_draft_authority_mismatch');
    ...
    const snapshot: ChangesetPlanningSnapshot = Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== eventSettingsReadPort) {
          throw new TypeError('event_settings_update_draft_undeclared_read_port');
        }
    ...
    return sealEventSettingsUpdateDraftPreparation({
      capability,
      context,
      preparation: {
        prepare: ({ businessInput, context: receivedContext }) => {
          if (receivedContext !== context || !this.input.sqlite.inTransaction) {
            throw new TypeError('event_settings_update_draft_context_substitution');
          }
          const business = eventSettingsUpdateDraftInputSchema.parse(businessInput);
          ...
          const repository = new SQLiteEventSettingsRepository(this.input.sqlite);
          const current = repository.readCurrentEventSettings(this.input.workspaceId);
          if (!current) {
          ...
          const before = repository.requireEventSettings(scope);
          const approvalPolicy = captureEventSettingsChangesetApprovalPolicy({
            bundle: this.#bundle
          });
          ...
          try {
            appended = appendChangesetDraftSynchronous({
              ...
              operations: [{
                ...
                authorInput: { scope, request: business }
              }],
              dependencyGroups: [{ key: 'event_settings', dependsOn: [] }],
              approvalPolicy,
              origin: 'human_ui'
            });
          } catch (error) {
            if (error instanceof EventSettingsPlanningError) {
          ...
          if (appended.kind === 'refused') {
            if (appended.refusal.kind !== 'id_collision') {
              throw new TypeError('event_settings_update_draft_unexpected_lifecycle_refusal');
            }
            this.#nonterminalReleaseContext = context;
            return collisionContribution();
          }
          const after = repository.requireEventSettings(scope);
          if (canonicalJsonText(after) !== canonicalJsonText(before)) {
            throw new TypeError('event_settings_update_draft_mutated_effective_state');
          }
          ...
          if (!revision || !operation
            ...
            throw new TypeError('event_settings_update_draft_record_incoherent');
          }
          const safeDiff = eventSettingsSafeDiffSchema.parse(operation.safeDiff);
          const candidate = eventSettingsUpdateDraftContributionSchema.parse({
            result: {
              kind: 'success',
              data: {
                schemaVersion: 1,
            ...
            domain: {
              kind: 'event_settings_changeset_draft',
          ...
          if (candidate.result.kind !== 'success' || candidate.domain === null) {
            throw new TypeError('event_settings_update_draft_success_contribution_invalid');
          }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    const parsed = eventSettingsUpdateDraftDomainContributionSchema.parse(contribution);
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    const parsedResult = eventSettingsUpdateDraftOperationResultSchema.safeParse(receipt.result);
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
        ...
        || receipt.ref.operationName !== EVENT_SETTINGS_UPDATE_DRAFT_OPERATION.name
        || receipt.ref.operationVersion !== EVENT_SETTINGS_UPDATE_DRAFT_OPERATION.version
        || !parsedResult.success || parsedResult.data.kind !== 'success'
        || parsedResult.data.receipt.id !== receipt.ref.id
        || parsedResult.data.receipt.operationName !== EVENT_SETTINGS_UPDATE_DRAFT_OPERATION.name
        || parsedResult.data.receipt.operationVersion !== EVENT_SETTINGS_UPDATE_DRAFT_OPERATION.version
        || canonicalJsonText(parsedResult.data.data)
          !== canonicalJsonText(active.contribution.result.data)) {
      throw new TypeError('event_settings_update_draft_receipt_mismatch');
    }
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('event_settings_update_draft_receipt_parent_missing');
    }
    const child = eventSettingsUpdateDraftEvidenceChildSchema.parse(contribution);
    const expected = active.contribution.receiptChildren[0];
    if (canonicalJsonText(child) !== canonicalJsonText(expected)) {
      throw new TypeError('event_settings_update_draft_evidence_mismatch');
    }
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('event_settings_update_draft_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
...
export function createSQLiteEventSettingsUpdateDraftEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly policy: EventOrdinaryPolicy;
  readonly ids: SQLiteEventSettingsUpdateDraftEffectIds;
}): {
  readonly capability: typeof EVENT_SETTINGS_UPDATE_DRAFT_HANDLER_CAPABILITY;
  readonly adapter: SQLiteEventSettingsUpdateDraftEffectDomainAdapter;
} {
  return Object.freeze({
    capability: EVENT_SETTINGS_UPDATE_DRAFT_HANDLER_CAPABILITY,
    adapter: new SQLiteEventSettingsUpdateDraftEffectDomainAdapter(input)
  });
}
