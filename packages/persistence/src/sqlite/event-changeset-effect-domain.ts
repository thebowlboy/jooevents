...
import {
  EVENT_CREATION_CHANGESET_KIND,
  EVENT_CREATION_CHANGESET_VERSION,
  assertEventCreationOrdinaryChangesetBundle,
  assertEventOrdinaryPolicy,
  captureRegisteredEventDependencies,
  createEventCreationOrdinaryChangesetBundle,
  createEventCreationValidationView,
  eventAggregateId,
  eventCreationReadPort,
  eventCreationTransactionPort,
  eventCreationValidationPort,
  type EventCreationChangesetPlan,
  type EventCreationOrdinaryChangesetBundle,
  type EventCreationReadPort,
  type EventCreationTransactionPort,
...
export const EVENT_CREATION_CHANGESET_EFFECT_SQL = `
CREATE TABLE event_creation_changeset_receipt_links (
...
CREATE TABLE event_creation_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind IN ('event_created', 'event_creation_compensated')),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES event_creation_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE event_creation_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES event_creation_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE event_creation_changeset_timeline (
...
CREATE TRIGGER event_creation_changeset_receipt_links_no_update
BEFORE UPDATE ON event_creation_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle receipt links are immutable'); END;
CREATE TRIGGER event_creation_changeset_receipt_links_no_delete
BEFORE DELETE ON event_creation_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle receipt links are immutable'); END;
CREATE TRIGGER event_creation_changeset_domain_facts_no_update
BEFORE UPDATE ON event_creation_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle facts are immutable'); END;
CREATE TRIGGER event_creation_changeset_domain_facts_no_delete
BEFORE DELETE ON event_creation_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle facts are immutable'); END;
CREATE TRIGGER event_creation_changeset_outbox_pointers_no_update
BEFORE UPDATE ON event_creation_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle pointers are immutable'); END;
CREATE TRIGGER event_creation_changeset_outbox_pointers_no_delete
BEFORE DELETE ON event_creation_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle pointers are immutable'); END;
CREATE TRIGGER event_creation_changeset_timeline_no_update
BEFORE UPDATE ON event_creation_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle timeline is immutable'); END;
CREATE TRIGGER event_creation_changeset_timeline_no_delete
BEFORE DELETE ON event_creation_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'changeset lifecycle timeline is immutable'); END;
`;

export function installEventCreationChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('changeset_effect_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(EVENT_CREATION_CHANGESET_EFFECT_SQL)).immediate();
}
...
export function createSQLiteEventCreationChangesetLifecycleStore(
  sqlite: Database
): SQLiteChangesetLifecycleStore {
  return new SQLiteChangesetLifecycleStore(sqlite, terminalReceiptSource(sqlite));
}

export interface SQLiteEventCreationChangesetEffectIds extends ChangesetLifecycleIds {
...
export interface EventCreationApproverAuthoritySource {
...
function exactSubjects(context: EffectInvocationContext): boolean {
  return context.scope.eventId === undefined
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === 'event_creation'
    );
}

function eventIdForPlan(plan: EventCreationChangesetPlan): string {
  return plan.mutation.action === 'create'
    ? plan.mutation.after.id
    : plan.mutation.before.id;
}

function eventIdForRecord(input: {
  readonly bundle: EventCreationOrdinaryChangesetBundle;
  readonly record: StoredChangesetRecord;
}): string | undefined {
  ...
  for (const revision of input.record.revisions) {
    for (const operation of revision.revision.operations) {
      ...
      const candidate = eventIdForPlan(plan);
      if (eventId !== undefined && eventId !== candidate) return undefined;
      eventId = candidate;
    }
  }
  return eventId;
...
function operationPlan(input: {
...
}): EventCreationChangesetPlan | undefined {
  if (input.operation.kind !== EVENT_CREATION_CHANGESET_KIND
      || input.operation.version !== EVENT_CREATION_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as EventCreationChangesetPlan | undefined;
}

function ownsEventCreationChangeset(input: {
  readonly bundle: EventCreationOrdinaryChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
function approvalPolicy(input: {
  ...
  revision.revision.operations.forEach((operation) => {
    const plan = operationPlan({ bundle: input.bundle, operation });
  ...
  return Object.freeze({
    ...
    requirement: input.bundle.policy.approval
  });
}
...
/**
 * Owns ordinary workspace-scoped Event creation changeset transitions and commit
 * on one caller-owned SQLite handle. Every lifecycle and domain write remains in
 * the Foundation effect unit of work.
 */
export class SQLiteEventCreationChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle: EventCreationOrdinaryChangesetBundle;
  readonly #ids: SQLiteEventCreationChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    /** Optional structural state initialized immediately after a new Event root. */
    readonly createdEventInitializer?: {
    ...
    readonly ids: SQLiteEventCreationChangesetEffectIds;
    readonly approverAuthority?: EventCreationApproverAuthoritySource;
  }) {
    ...
    if (input.createdEventInitializer !== undefined
      ...
      throw new TypeError('changeset_effect_event_initializer_invalid');
    }
    this.#bundle = createEventCreationOrdinaryChangesetBundle({
      dependencyRegistry: input.dependencyRegistry,
      policy: input.policy
    });
    assertEventCreationOrdinaryChangesetBundle(this.#bundle);
    this.lifecycleStore = createSQLiteEventCreationChangesetLifecycleStore(input.sqlite);
    for (const method of [
    ...
    ) as unknown as SQLiteEventCreationChangesetEffectIds);
    this.subjectRelationships = Object.freeze({
      validateSubject: ({
        ...
        const row = sqlite.query<{ readonly workspace_id: string }, [string]>(`
          SELECT workspace_id FROM event_spine_workspace_sets
           WHERE workspace_id = ?
           LIMIT 2
        `).all(workspaceId);
        return row.length === 1 && row[0]?.workspace_id === workspaceId
          ? Object.freeze({
              kind: 'valid' as const,
              evidenceIds: Object.freeze([
                `changeset-owner:event_creation:${workspaceId}:${this.#bundle.policy.definitionDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsEventCreationChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: 'event_creation',
      evidenceIds: Object.freeze([
        `event-creation-policy:${this.#bundle.policy.definitionDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: 'event_creation'
      },
      evaluatedAt
    });
    if (subjectRelationship.kind !== 'valid') {
  ...
  private nextId(method: keyof SQLiteEventCreationChangesetEffectIds): string {
  ...
  private repository(): EventCreationTransactionPort {
    const spine = new SQLiteEventSpineRepository(this.input.sqlite);
    let port: EventCreationTransactionPort;
    port = Object.freeze({
      readEventSet: (workspaceId: string) => spine.readEventSet(workspaceId),
      readEvent: (workspaceId: string, eventId: string) => spine.readEvent(workspaceId, eventId),
      readContributor: (
        contributor: Parameters<EventDependencySnapshotSource['readContributor']>[0],
        scope: Parameters<EventDependencySnapshotSource['readContributor']>[1]
      ) => this.input.dependencySource.readContributor(contributor, scope),
      applyEventCreatePlan: (plan: EventCreatePlan) => {
        const result = spine.applyEventCreatePlan(plan);
      ...
      applyEventCreateCompensationPlan: (plan: EventCreateCompensationPlan) =>
        spine.applyEventCreateCompensationPlan({
          plan,
          dependencyRegistry: this.input.dependencyRegistry,
          readPort: port
        })
    });
    return port;
  ...
  private planningSnapshot(repository: EventCreationReadPort): ChangesetPlanningSnapshot {
    const view = createEventCreationValidationView(repository);
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== eventCreationReadPort) {
  ...
  private commitTransaction(repository: EventCreationTransactionPort): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        if ((key as unknown) !== eventCreationValidationPort
            && (key as unknown) !== eventCreationTransactionPort) {
  ...
  private currentCommitEvidence(input: {
    readonly record: StoredChangesetRecord;
    readonly repository: EventCreationTransactionPort;
  }) {
    const eventId = eventIdForRecord({ bundle: this.#bundle, record: input.record });
    if (eventId === undefined) throw new TypeError('changeset_effect_event_missing');
    const eventSet = input.repository.readEventSet(input.record.head.workspaceId);
    if (!eventSet) throw new TypeError('changeset_effect_domain_missing');
    const aggregateVersions = new Map<string, number>();
    const event = input.repository.readEvent(input.record.head.workspaceId, eventId);
    if (event) aggregateVersions.set(eventAggregateId(event.id), event.version);
    const guardVersions = new Map<string, number>([
      [workspaceEventSetGuardId(input.record.head.workspaceId), eventSet.version]
    ]);
    const guardDigests = new Map<string, string>([
      [workspaceEventSetGuardId(input.record.head.workspaceId), workspaceEventSetDigest(eventSet)]
    ]);
    const dependencies = captureRegisteredEventDependencies({
      registry: this.input.dependencyRegistry,
      scope: {
    ...
    for (const contributor of dependencies.contributors) {
  ...
  private exactRecord(changesetId: string): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === 'event_creation'
      ? record
      : undefined;
  }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!this.exactRecord(wire.changesetId)) {
    ...
    if (!revision) throw new TypeError('changeset_effect_revision_missing');
    const eventId = eventIdForRecord({ bundle: this.#bundle, record });
    if (eventId === undefined) throw new TypeError('changeset_effect_event_missing');
    const planned = operationPlan({
      bundle: this.#bundle,
      operation: revision.revision.operations[0]!
    });
    if (!planned) throw new TypeError('changeset_effect_owner_mismatch');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: planned.mutation.action === 'create'
            ? 'event_created'
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO event_creation_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO event_creation_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO event_creation_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO event_creation_changeset_timeline (
...
export function createSQLiteEventCreationChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly policy: EventOrdinaryPolicy;
  readonly dependencyRegistry: EventDependencyContributorRegistry;
  ...
  const adapter = new SQLiteEventCreationChangesetEffectDomainAdapter(input);
  return Object.freeze({
