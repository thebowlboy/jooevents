...
import {
  ...
  taskMembershipValidationPort,
  taskReadPort,
  taskTransactionPort,
  taskValidationPort,
  TASK_CHANGESET_KIND,
  TASK_CHANGESET_VERSION,
  type TaskChangesetBundle
...
import type { SQLiteDeadlineRepository } from './deadline';
import type { SQLiteTaskRepository } from './tasks';

/** Stable changeset-owner identity for every Task core changeset. */
...
export const TASK_CHANGESET_COMMIT_FACT_KIND = 'task_core_changed' as const;

export const TASK_CHANGESET_EFFECT_SQL = `
CREATE TABLE task_changeset_receipt_links (
...
CREATE TABLE task_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'task_core_changed'),
...
CREATE TRIGGER task_changeset_receipt_links_no_update
BEFORE UPDATE ON task_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'task changeset receipt links are immutable'); END;
CREATE TRIGGER task_changeset_receipt_links_no_delete
BEFORE DELETE ON task_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'task changeset receipt links are immutable'); END;
CREATE TRIGGER task_changeset_domain_facts_no_update
BEFORE UPDATE ON task_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'task changeset facts are immutable'); END;
CREATE TRIGGER task_changeset_domain_facts_no_delete
BEFORE DELETE ON task_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'task changeset facts are immutable'); END;
CREATE TRIGGER task_changeset_outbox_pointers_no_update
BEFORE UPDATE ON task_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'task changeset pointers are immutable'); END;
CREATE TRIGGER task_changeset_outbox_pointers_no_delete
BEFORE DELETE ON task_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'task changeset pointers are immutable'); END;
CREATE TRIGGER task_changeset_timeline_no_update
BEFORE UPDATE ON task_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'task changeset timeline is immutable'); END;
CREATE TRIGGER task_changeset_timeline_no_delete
BEFORE DELETE ON task_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'task changeset timeline is immutable'); END;
`;

export function installTaskChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('task_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(TASK_CHANGESET_EFFECT_SQL)).immediate();
}

export interface SQLiteTaskChangesetEffectIds extends ChangesetLifecycleIds {
...
const TASK_PERMISSION_IDS: readonly PermissionId[] = Object.freeze([
  'event.manage'
]);
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`task_changeset_${label}_invalid`);
  }
  return value;
}
...
function exactSubjects(context: EffectInvocationContext, eventId: EventId): boolean {
  return context.scope.eventId === eventId
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === TASK_CHANGESET_OWNER_ID
    );
}

function operationPlan(input: {
  readonly bundle: TaskChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): TaskMutationPlanDto | TaskAssignmentRestorePlanDto | undefined {
  if (input.operation.kind !== TASK_CHANGESET_KIND
      || input.operation.version !== TASK_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as
    | TaskMutationPlanDto | TaskAssignmentRestorePlanDto | undefined;
}

function ownsTaskChangeset(input: {
  readonly bundle: TaskChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
/**
 * Owns the Task changeset transitions and the effective Task commit on one
 * caller-owned SQLite handle. Every lifecycle, Task, and collaborating
 * `task_due` Deadline write stays inside the Foundation effect unit of work:
 * the planning snapshot and commit transaction serve the Task ports and the
 * three Deadline collaboration ports from the one composed Task repository, so
 * opening a round creates its Deadline atomically with the round or not at all.
 */
export class SQLiteTaskChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #ids: SQLiteTaskChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    readonly repository: SQLiteTaskRepository;
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:task:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsTaskChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: TASK_CHANGESET_OWNER_ID,
      evidenceIds: Object.freeze([
        `task-definition:${this.#bundle.registry.registryDigestSha256}`
      ]),
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) throw new TypeError('task_changeset_transaction_required');
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('task_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') throw new TypeError('task_changeset_approval_not_mounted');
    ...
    if (action === undefined
      ...
      throw new TypeError('task_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        || !authority.grants.some((grant) =>
          ...
          && TASK_PERMISSION_IDS.includes(grant.key as PermissionId)
        )) throw new TypeError('task_changeset_authority_mismatch');
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: TASK_CHANGESET_OWNER_ID
      },
      evaluatedAt
    });
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('task_changeset_relationship_mismatch');
    }
    this.clearTransient();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('task_changeset_context_substitution');
          }
          return this.prepare({
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('task_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('task_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('task_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
      ...
      const validation = validateStoredChangesetCommit({
        ...
        context: actorContext,
        ...wire,
        currentApprovalPolicy: TASK_DRAFT_APPROVAL_POLICY,
      ...
      const valid = kinds.length === 1 && kinds[0] === 'task_assignment_changed'
        || kinds.length === 3
          && kinds[0] === 'deadline_changed'
          && kinds[1] === 'task_definition_created'
          && kinds[2] === 'task_assignments_materialized';
      if (!valid || facts.some((fact) => fact.version !== 1)) {
        throw new TypeError('task_changeset_fact_contribution_invalid');
      }
    ...
    if (!revision) throw new TypeError('task_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: TASK_CHANGESET_COMMIT_FACT_KIND,
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('task_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('task_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('task_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('task_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO task_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('task_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('task_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO task_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO task_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO task_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('task_changeset_incomplete');
      }
  ...
  private nextId(method: keyof SQLiteTaskChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('task_changeset_ids_not_unique');
  ...
  private recordPermissionIds(
    ...
    for (const revision of record.revisions) {
      for (const operation of revision.revision.operations) {
        ...
        if (!plan) throw new TypeError('task_changeset_owner_mismatch');
      }
    }
    return ['event.manage'];
  }
  ...
  private planningSnapshot(repository: SQLiteTaskRepository): ChangesetPlanningSnapshot {
    const deadlines = this.input.deadlines;
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        ...
        throw new TypeError('task_changeset_undeclared_read_port');
      }
    });
  }

  private commitTransaction(repository: SQLiteTaskRepository): ChangesetCommitTransaction {
    const deadlines = this.input.deadlines;
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        if ((key as unknown) === taskValidationPort
            || (key as unknown) === taskMembershipValidationPort
            || (key as unknown) === taskTransactionPort) {
          return repository as unknown as Port;
        }
        if ((key as unknown) === taskDueDeadlineValidationPort
            || (key as unknown) === taskDueDeadlineTransactionPort) {
          return deadlines as unknown as Port;
        }
        throw new TypeError('task_changeset_undeclared_transaction_port');
      }
    });
  }

  /**
   * Current-state evidence for exact-commit validation: the task catalog,
   * candidate query, tasker query, and Deadline catalog guards; the event and
   * every referenced `deadline:<id>` aggregate; per-subject task aggregates;
   * and the `task_head_absence` guard, published only while the head is truly
   * absent so a committed task always surfaces as `guard_changed`.
   */
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('task_changeset_event_required');
    const scope: TaskScopeDto = {
      workspaceId: parseWorkspaceId(input.record.head.workspaceId),
      eventId: parseEventId(eventId)
    };
    const repository = input.repository;
    const catalog = repository.readTaskCatalog(scope);
    if (!catalog) throw new TypeError('task_changeset_domain_missing');
    ...
    guardVersions.set(taskCatalogGuardId(eventId), catalog.version);
    guardDigests.set(taskCatalogGuardId(eventId), catalog.digestSha256);

    const memberships = repository.listConfirmedTaskEngagements(scope);
    guardVersions.set(taskMembershipGuardId(eventId), 1);
    guardDigests.set(taskMembershipGuardId(eventId), canonicalJsonSha256({
      scope, rule: 'all_confirmed_speakers@1', evidence: memberships
    }));
    const deadlineCatalog = this.input.deadlines.readDeadlineCatalog(scope);
    if (deadlineCatalog) {
    ...
    const eventTimeBasis = this.input.deadlines.readDeadlineEventTimeBasis(scope);
    if (eventTimeBasis) aggregateVersions.set(`event:${eventId}`, eventTimeBasis.eventVersion);

    const head = input.record.revisions.at(-1);
    if (!head) throw new TypeError('task_changeset_revision_missing');
    for (const operation of head.revision.operations) {
      ...
      for (const ref of operation.aggregateRefs) {
        ...
        if (kind === 'task_assignment') {
          const current = repository.readTaskAssignment(scope, id);
   ...
   * scope, resolves to the Task owner, and the caller holds every per-action
   ...
   * partially privileged operator cannot distinguish a Task changeset above
  ...
  private exactRecord(
    ...
    if (!record
        ...
        || this.resolveOwner(record)?.id !== TASK_CHANGESET_OWNER_ID
...
export function createSQLiteTaskChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly repository: SQLiteTaskRepository;
  readonly deadlines: SQLiteDeadlineRepository;
  ...
  const adapter = new SQLiteTaskChangesetEffectDomainAdapter(input);
  return Object.freeze({
    ownerId: TASK_CHANGESET_OWNER_ID,
