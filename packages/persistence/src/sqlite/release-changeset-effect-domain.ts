...
import type { ReleaseMutationPlanDto, ReleaseScopeDto, SurfaceKind } from '@jooevents/contracts';
...
import {
  ...
  releaseChainGuard,
  releaseReadPort,
  releaseTransactionPort,
  releaseValidationPort,
  surfaceHeadGuard,
  ReleasePlanningError,
  RELEASE_CHANGESET_COMMIT_FACT_KIND,
  RELEASE_CHANGESET_KIND,
  RELEASE_CHANGESET_OWNER_ID,
  RELEASE_CHANGESET_VERSION,
  type ReleaseChangesetBundle
...
export const SQLITE_RELEASE_CHANGESET_EFFECT_SQL = `
CREATE TABLE release_changeset_receipt_links (
...
CREATE TABLE release_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'release_changed'),
...
CREATE TRIGGER release_changeset_receipt_links_no_update
BEFORE UPDATE ON release_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'release changeset receipt links are immutable'); END;
CREATE TRIGGER release_changeset_receipt_links_no_delete
BEFORE DELETE ON release_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'release changeset receipt links are immutable'); END;
CREATE TRIGGER release_changeset_domain_facts_no_update
BEFORE UPDATE ON release_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'release changeset facts are immutable'); END;
CREATE TRIGGER release_changeset_domain_facts_no_delete
BEFORE DELETE ON release_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'release changeset facts are immutable'); END;
CREATE TRIGGER release_changeset_outbox_pointers_no_update
BEFORE UPDATE ON release_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'release changeset pointers are immutable'); END;
CREATE TRIGGER release_changeset_outbox_pointers_no_delete
BEFORE DELETE ON release_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'release changeset pointers are immutable'); END;
CREATE TRIGGER release_changeset_timeline_no_update
BEFORE UPDATE ON release_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'release changeset timeline is immutable'); END;
CREATE TRIGGER release_changeset_timeline_no_delete
BEFORE DELETE ON release_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'release changeset timeline is immutable'); END;
`;

export function installReleaseChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('release_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(SQLITE_RELEASE_CHANGESET_EFFECT_SQL)).immediate();
}

export interface SQLiteReleaseChangesetEffectIds extends ChangesetLifecycleIds {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`release_changeset_${label}_invalid`);
  }
  return value;
}
...
function exactSubjects(context: EffectInvocationContext, eventId: EventId): boolean {
  return context.scope.eventId === eventId
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === RELEASE_CHANGESET_OWNER_ID
    );
}

function planScope(plan: ReleaseMutationPlanDto): ReleaseScopeDto {
  return plan.input.scope;
}

function operationPlan(input: {
  readonly bundle: ReleaseChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): ReleaseMutationPlanDto | undefined {
  if (input.operation.kind !== RELEASE_CHANGESET_KIND
      || input.operation.version !== RELEASE_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as ReleaseMutationPlanDto | undefined;
}

function ownsReleaseChangeset(input: {
  readonly bundle: ReleaseChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
/**
 * Owns the release changeset lifecycle and the effective publish commit on one
 * caller-owned SQLite handle. Every lifecycle write and every release row
 * stays inside the Foundation effect unit of work; the planning snapshot and
 * commit transaction serve the release ports from the one release repository.
 * Commits emit facts only — no serving path and no messaging effect ever
 * originates in this transaction, and no model output can reach a release row
 * except through a reviewed committed plan.
 */
export class SQLiteReleaseChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle = createReleaseChangesetBundle();
  readonly #ids: SQLiteReleaseChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    /** The release draft approval policy, supplied by the operations layer. */
    readonly approvalPolicy: CapturedChangesetApprovalPolicy;
    /** The release publication permission, supplied by the operations layer. */
    ...
    for (const method of ID_METHODS) {
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('release_changeset_id_factory_invalid');
      }
    }
    this.#ids = Object.freeze(Object.fromEntries(
      ID_METHODS.map((method) => [method, input.ids[method].bind(input.ids)])
    ) as unknown as SQLiteReleaseChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:release:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsReleaseChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: RELEASE_CHANGESET_OWNER_ID,
      evidenceIds: Object.freeze([
        `release-definition:${this.#bundle.registry.registryDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) throw new TypeError('release_changeset_transaction_required');
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('release_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') throw new TypeError('release_changeset_approval_not_mounted');
    ...
    if (action === undefined
      ...
      throw new TypeError('release_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        || !authority.grants.some((grant) =>
          grant.kind === 'permission' && grant.key === this.input.permissionId
        )) throw new TypeError('release_changeset_authority_mismatch');
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: RELEASE_CHANGESET_OWNER_ID
      },
      evaluatedAt
    });
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('release_changeset_relationship_mismatch');
    }
    this.clearTransient();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('release_changeset_context_substitution');
          }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('release_changeset_proposal_diff_missing');
      ...
      try {
        result = rebuildStoredChangesetSynchronous({
          ...
          groups: wire.groups,
          approvalPolicy: this.input.approvalPolicy
        });
      } catch (error) {
        if (!(error instanceof ReleasePlanningError)) throw error;
        this.#nonterminalReleaseContext = input.context;
      ...
      if (!diff) throw new TypeError('release_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('release_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
      ...
      const validation = validateStoredChangesetCommit({
        ...
        context: actorContext,
        ...wire,
        currentApprovalPolicy: this.input.approvalPolicy,
      ...
      if (facts.length !== 1
          || facts[0]?.kind !== RELEASE_CHANGESET_COMMIT_FACT_KIND
          || facts[0].version !== 1) {
        throw new TypeError('release_changeset_fact_contribution_invalid');
      }
    ...
    if (!revision) throw new TypeError('release_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: RELEASE_CHANGESET_COMMIT_FACT_KIND,
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('release_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('release_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('release_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('release_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO release_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('release_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('release_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO release_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO release_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO release_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('release_changeset_incomplete');
      }
  ...
  private nextId(method: keyof SQLiteReleaseChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('release_changeset_ids_not_unique');
    this.#issuedIds.add(value);
    return value;
  }

  private repository(): SQLiteReleaseRepository {
    return new SQLiteReleaseRepository(this.input.sqlite, this.input.sources);
  }

  private planningSnapshot(repository: SQLiteReleaseRepository): ChangesetPlanningSnapshot {
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== releaseReadPort) {
          throw new TypeError('release_changeset_undeclared_read_port');
        }
  ...
  private commitTransaction(repository: SQLiteReleaseRepository): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        if ((key as unknown) !== releaseValidationPort
            && (key as unknown) !== releaseTransactionPort) {
          throw new TypeError('release_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  /**
   * Current-state evidence for exact-commit validation: this domain publishes
   * guards only — chain and head fences that must also fence absence — and no
   * aggregate refs, since committed releases are immutable and cannot drift.
   * The guard values are recomputed here with the same domain helpers the
   * planner used, so both sides always agree on the same current state.
   */
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('release_changeset_event_required');
    const scope: ReleaseScopeDto = {
    ...
    if (!head) throw new TypeError('release_changeset_revision_missing');
    for (const operation of head.revision.operations) {
      const plan = operationPlan({ bundle: this.#bundle, operation });
      if (!plan) throw new TypeError('release_changeset_owner_mismatch');
      for (const ref of operation.guardRefs) {
        const guard = this.currentGuard(scope, ref.id, plan, input.repository);
        if (!guard) continue;
        guardVersions.set(ref.id, guard.version);
        guardDigests.set(ref.id, guard.digest);
      }
    }
  ...
  ): { readonly version: number; readonly digest: string } | undefined {
    if (guardId.startsWith('program_release_chain:')) {
      return releaseChainGuard(repository.readCurrentProgramRelease(scope));
    }
    if (guardId.startsWith('style_set_release_chain:')) {
      return releaseChainGuard(repository.readCurrentStyleSetRelease(scope));
    }
    if (guardId.startsWith('surface_head_state:')) {
      if (isProgramPlan(plan) || isStyleSetPlan(plan)) return undefined;
      const kind: SurfaceKind = plan.input.kind;
      return surfaceHeadGuard(repository.readSurfaceHead(scope, kind));
    }
    return undefined;
  }
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === RELEASE_CHANGESET_OWNER_ID
      ? record
      : undefined;
  }
}

export function createSQLiteReleaseChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly approvalPolicy: CapturedChangesetApprovalPolicy;
  readonly permissionId: PermissionId;
  ...
  const adapter = new SQLiteReleaseChangesetEffectDomainAdapter(input);
  return Object.freeze({
