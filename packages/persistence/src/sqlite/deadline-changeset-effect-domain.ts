...
import {
  ...
  parseDeadlineChangesetPlan,
  DEADLINE_CHANGESET_KIND,
  DEADLINE_CHANGESET_VERSION,
  type DeadlineChangesetBundle,
...
export const DEADLINE_CHANGESET_EFFECT_SQL = `
CREATE TABLE deadline_changeset_receipt_links (
...
CREATE TABLE deadline_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'deadline_changed'),
...
CREATE TRIGGER deadline_changeset_receipt_links_no_update
BEFORE UPDATE ON deadline_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'deadline changeset receipt links are immutable'); END;
CREATE TRIGGER deadline_changeset_receipt_links_no_delete
BEFORE DELETE ON deadline_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'deadline changeset receipt links are immutable'); END;
CREATE TRIGGER deadline_changeset_domain_facts_no_update
BEFORE UPDATE ON deadline_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'deadline changeset facts are immutable'); END;
CREATE TRIGGER deadline_changeset_domain_facts_no_delete
BEFORE DELETE ON deadline_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'deadline changeset facts are immutable'); END;
CREATE TRIGGER deadline_changeset_outbox_pointers_no_update
BEFORE UPDATE ON deadline_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'deadline changeset pointers are immutable'); END;
CREATE TRIGGER deadline_changeset_outbox_pointers_no_delete
BEFORE DELETE ON deadline_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'deadline changeset pointers are immutable'); END;
CREATE TRIGGER deadline_changeset_timeline_no_update
BEFORE UPDATE ON deadline_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'deadline changeset timeline is immutable'); END;
CREATE TRIGGER deadline_changeset_timeline_no_delete
BEFORE DELETE ON deadline_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'deadline changeset timeline is immutable'); END;
`;

export function installDeadlineChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('deadline_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(DEADLINE_CHANGESET_EFFECT_SQL)).immediate();
}
...
export interface SQLiteDeadlineChangesetEffectIds extends ChangesetLifecycleIds {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`deadline_changeset_${label}_invalid`);
  }
  return value;
}
...
function operationPlan(input: {
  readonly bundle: DeadlineChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): DeadlineChangesetPlan | undefined {
  if (input.operation.kind !== DEADLINE_CHANGESET_KIND
      || input.operation.version !== DEADLINE_CHANGESET_VERSION) return undefined;
  ...
  return parseDeadlineChangesetPlan(input.operation.plan);
}

function deadlinePlan(input: {
  readonly bundle: DeadlineChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): DeadlineChangesetPlan | undefined {
...
function ownsDeadlineChangeset(input: {
  readonly bundle: DeadlineChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
  ...
  for (const revision of input.record.revisions) {
    for (const operation of revision.revision.operations) {
      const plan = deadlinePlan({ bundle: input.bundle, operation });
      if (!plan
          || plan.mutation.input.scope.workspaceId !== input.record.head.workspaceId
          || plan.mutation.input.scope.eventId !== input.record.head.eventId) return false;
    }
  }
  return true;
}
...
export class SQLiteDeadlineChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle = createDeadlineChangesetBundle();
  readonly #ids: SQLiteDeadlineChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    for (const method of [
      ...
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('deadline_changeset_id_factory_invalid');
      }
    }
    ...
    ) as unknown as SQLiteDeadlineChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:deadline:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsDeadlineChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: 'deadline',
      evidenceIds: Object.freeze([
        `deadline-definition:${this.#bundle.registry.registryDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) throw new TypeError('deadline_changeset_transaction_required');
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('deadline_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') throw new TypeError('deadline_changeset_approval_not_mounted');
    ...
    if (action === undefined
      ...
      throw new TypeError('deadline_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        )) throw new TypeError('deadline_changeset_authority_mismatch');
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: 'deadline'
      },
      evaluatedAt
    });
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('deadline_changeset_relationship_mismatch');
    }
    this.#prepared.clear();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('deadline_changeset_context_substitution');
          }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('deadline_changeset_proposal_diff_missing');
      ...
      const result = rebuildStoredChangesetSynchronous({
        ...
        snapshot: this.planningSnapshot(repository, input.actorUserId, input.evaluatedAt),
        ids,
      ...
      if (!diff) throw new TypeError('deadline_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('deadline_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
      ...
      const validation = validateStoredChangesetCommit({
        ...
        context: actorContext,
        ...wire,
        currentApprovalPolicy: DEADLINE_DRAFT_APPROVAL_POLICY,
      ...
      if (facts.length !== 1
          || facts[0]?.kind !== 'deadline_changed'
          || facts[0].version !== 1) {
        throw new TypeError('deadline_changeset_fact_contribution_invalid');
      }
    ...
    if (!revision) throw new TypeError('deadline_changeset_revision_missing');
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('deadline_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('deadline_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('deadline_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('deadline_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO deadline_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('deadline_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('deadline_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO deadline_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO deadline_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO deadline_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('deadline_changeset_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
  ...
  private nextId(method: keyof SQLiteDeadlineChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('deadline_changeset_ids_not_unique');
  ...
  private planningSnapshot(
    ...
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        ...
        if ((key as unknown) === deadlinePlanningAttributionReadPort) {
          return Object.freeze({
            readDeadlinePlanningAttribution: () => Object.freeze({
              userId: actorUserId,
              at: evaluatedAt
            })
          }) as unknown as Port;
        }
        throw new TypeError('deadline_changeset_undeclared_read_port');
      }
    });
  }

  private commitTransaction(repository: SQLiteDeadlineRepository): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        if ((key as unknown) !== deadlineChangesetValidationPort
            && (key as unknown) !== deadlineChangesetTransactionPort) {
          throw new TypeError('deadline_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('deadline_changeset_event_required');
    const scope = {
      workspaceId: parseWorkspaceId(input.record.head.workspaceId),
      eventId: parseEventId(eventId)
    };
    const catalog = input.repository.readDeadlineCatalog(scope);
    const eventTimeBasis = input.repository.readDeadlineEventTimeBasis(scope);
    if (!catalog || !eventTimeBasis) throw new TypeError('deadline_changeset_domain_missing');
    const aggregateVersions = new Map<string, number>([
      [deadlineEventAggregateId(eventId), eventTimeBasis.eventVersion],
      ...catalog.deadlines.map((deadline) => [
        deadlineAggregateId(deadline.id), deadline.version
      ] as const)
    ]);
    const guardVersions = new Map<string, number>([
      [deadlineCatalogGuardId(eventId), catalog.version]
    ]);
    const guardDigests = new Map<string, string>([
      [deadlineCatalogGuardId(eventId), catalog.digestSha256]
    ]);
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === 'deadline'
      ? record
      : undefined;
  }
}

export function createSQLiteDeadlineChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly eventRelationships: SQLiteOperatorEventRelationshipSource;
  readonly ids: SQLiteDeadlineChangesetEffectIds;
}) {
  const adapter = new SQLiteDeadlineChangesetEffectDomainAdapter(input);
  return Object.freeze({
