...
import {
  ...
  fieldRegistryTransactionPort,
  fieldRegistryValidationPort,
  type FieldRegistryChangesetPlan,
...
export const FIELD_REGISTRY_CHANGESET_EFFECT_SQL = `
CREATE TABLE field_registry_changeset_receipt_links (
...
CREATE TABLE field_registry_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'field_registry_changed'),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES field_registry_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE field_registry_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES field_registry_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE field_registry_changeset_timeline (
...
CREATE TRIGGER field_registry_changeset_receipt_links_no_update
BEFORE UPDATE ON field_registry_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'field registry changeset receipt links are immutable'); END;
CREATE TRIGGER field_registry_changeset_receipt_links_no_delete
BEFORE DELETE ON field_registry_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'field registry changeset receipt links are immutable'); END;
CREATE TRIGGER field_registry_changeset_domain_facts_no_update
BEFORE UPDATE ON field_registry_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'field registry changeset facts are immutable'); END;
CREATE TRIGGER field_registry_changeset_domain_facts_no_delete
BEFORE DELETE ON field_registry_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'field registry changeset facts are immutable'); END;
CREATE TRIGGER field_registry_changeset_outbox_pointers_no_update
BEFORE UPDATE ON field_registry_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'field registry changeset pointers are immutable'); END;
CREATE TRIGGER field_registry_changeset_outbox_pointers_no_delete
BEFORE DELETE ON field_registry_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'field registry changeset pointers are immutable'); END;
CREATE TRIGGER field_registry_changeset_timeline_no_update
BEFORE UPDATE ON field_registry_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'field registry changeset timeline is immutable'); END;
CREATE TRIGGER field_registry_changeset_timeline_no_delete
BEFORE DELETE ON field_registry_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'field registry changeset timeline is immutable'); END;
`;

export function installFieldRegistryChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('field_registry_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(FIELD_REGISTRY_CHANGESET_EFFECT_SQL)).immediate();
}
...
export function createSQLiteFieldRegistryChangesetLifecycleStore(
  sqlite: Database
): SQLiteChangesetLifecycleStore {
  return new SQLiteChangesetLifecycleStore(sqlite, terminalReceiptSource(sqlite));
}

export interface SQLiteFieldRegistryChangesetEffectIds extends ChangesetLifecycleIds {
...
export interface FieldRegistryApproverAuthoritySource {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`field_registry_changeset_${label}_invalid`);
  }
  return value;
}
...
function operationPlan(input: {
...
}): FieldRegistryChangesetPlan | undefined {
  if (input.operation.kind !== FIELD_REGISTRY_CHANGESET_KIND
      || input.operation.version !== FIELD_REGISTRY_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as FieldRegistryChangesetPlan | undefined;
}

function ownsFieldRegistryChangeset(input: {
  readonly bundle: FieldRegistryOrdinaryChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
function approvalPolicy(bundle: FieldRegistryOrdinaryChangesetBundle) {
  return Object.freeze({
...
/** Owns generic lifecycle transitions and atomic effective Field Registry commit. */
export class SQLiteFieldRegistryChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle: FieldRegistryOrdinaryChangesetBundle;
  readonly #ids: SQLiteFieldRegistryChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    readonly ids: SQLiteFieldRegistryChangesetEffectIds;
    readonly approverAuthority?: FieldRegistryApproverAuthoritySource;
  }) {
    ...
    this.lifecycleStore = createSQLiteFieldRegistryChangesetLifecycleStore(input.sqlite);
    for (const method of [
      ...
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('field_registry_changeset_id_factory_invalid');
      }
    }
    ...
    ) as unknown as SQLiteFieldRegistryChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:field_registry:${eventId}:${this.#bundle.policy.definitionDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsFieldRegistryChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: 'field_registry',
      evidenceIds: Object.freeze([
        `field-registry-policy:${this.#bundle.policy.definitionDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) {
      throw new TypeError('field_registry_changeset_transaction_required');
    }
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('field_registry_changeset_capability_mismatch');
    }
    ...
    if (action === undefined
      ...
      throw new TypeError('field_registry_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
      ...
      throw new TypeError('field_registry_changeset_authority_mismatch');
    }
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: 'field_registry'
      },
      evaluatedAt
    });
    if (eventRelationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('field_registry_changeset_relationship_mismatch');
    }
    this.#prepared.clear();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('field_registry_changeset_context_substitution');
          }
  ...
  private nextId(method: keyof SQLiteFieldRegistryChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) {
      throw new TypeError('field_registry_changeset_ids_not_unique');
    }
    this.#issuedIds.add(value);
    return value;
  }

  private repository(): SQLiteFieldRegistryRepository {
  ...
  private planningSnapshot(repository: SQLiteFieldRegistryRepository): ChangesetPlanningSnapshot {
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== fieldRegistryReadPort) {
          throw new TypeError('field_registry_changeset_undeclared_read_port');
        }
  ...
  private commitTransaction(repository: SQLiteFieldRegistryRepository): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(
        key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>
      ): Port {
        if ((key as unknown) !== fieldRegistryValidationPort
            && (key as unknown) !== fieldRegistryTransactionPort) {
          throw new TypeError('field_registry_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('field_registry_changeset_event_required');
    const state = input.repository.readFieldRegistry({
      workspaceId: input.record.head.workspaceId,
      eventId
    });
    if (!state) throw new TypeError('field_registry_changeset_domain_missing');
    const aggregateVersions = new Map<string, number>([
      [fieldRegistryAggregateId(eventId), state.version]
    ]);
    for (const revision of input.record.revisions) {
      for (const operation of revision.revision.operations) {
        const plan = operationPlan({ bundle: this.#bundle, operation });
        if (!plan) throw new TypeError('field_registry_changeset_owner_mismatch');
        const pin = plan.mutation.formPin;
        if (!pin) continue;
        const current = input.repository.resolveFormReference(state.scope, pin.id);
        if (current) aggregateVersions.set(`intake_form:${pin.id}`, current.version);
      }
    }
    const guardId = `field_registry_guard:${eventId}`;
    return Object.freeze({
      aggregateVersions,
      guardVersions: new Map<string, number>([[guardId, state.version]]),
      guardDigests: new Map<string, string>([[guardId, fieldRegistryGuardDigest(state)]])
    });
  }
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === 'field_registry'
      ? record
      : undefined;
  }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('field_registry_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('field_registry_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('field_registry_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
    ...
    if (!revision) throw new TypeError('field_registry_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: 'field_registry_changed',
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('field_registry_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('field_registry_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('field_registry_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('field_registry_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO field_registry_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('field_registry_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('field_registry_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO field_registry_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO field_registry_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO field_registry_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('field_registry_changeset_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
...
export function createSQLiteFieldRegistryChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly policy: FieldRegistryOrdinaryPolicy;
  readonly eventRelationships: SQLiteOperatorEventRelationshipSource;
  ...
  const adapter = new SQLiteFieldRegistryChangesetEffectDomainAdapter(input);
  return Object.freeze({
