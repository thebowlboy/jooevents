...
CREATE TABLE intake_form_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'intake_form_changed'),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES intake_form_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE intake_form_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES intake_form_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE intake_form_changeset_timeline (
...
CREATE TRIGGER intake_form_changeset_receipt_links_no_update
BEFORE UPDATE ON intake_form_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'intake form changeset receipt links are immutable'); END;
CREATE TRIGGER intake_form_changeset_receipt_links_no_delete
BEFORE DELETE ON intake_form_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'intake form changeset receipt links are immutable'); END;
CREATE TRIGGER intake_form_changeset_domain_facts_no_update
BEFORE UPDATE ON intake_form_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'intake form changeset facts are immutable'); END;
CREATE TRIGGER intake_form_changeset_domain_facts_no_delete
BEFORE DELETE ON intake_form_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'intake form changeset facts are immutable'); END;
CREATE TRIGGER intake_form_changeset_outbox_pointers_no_update
BEFORE UPDATE ON intake_form_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'intake form changeset pointers are immutable'); END;
CREATE TRIGGER intake_form_changeset_outbox_pointers_no_delete
BEFORE DELETE ON intake_form_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'intake form changeset pointers are immutable'); END;
CREATE TRIGGER intake_form_changeset_timeline_no_update
BEFORE UPDATE ON intake_form_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'intake form changeset timeline is immutable'); END;
CREATE TRIGGER intake_form_changeset_timeline_no_delete
BEFORE DELETE ON intake_form_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'intake form changeset timeline is immutable'); END;
`;

export const INTAKE_FORM_CHANGESET_OWNER_ID = 'intake_form' as const;

export function installIntakeFormChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('intake_form_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(INTAKE_FORM_CHANGESET_EFFECT_SQL)).immediate();
}
...
export interface SQLiteIntakeFormChangesetEffectIds extends ChangesetLifecycleIds {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`intake_form_changeset_${label}_invalid`);
  }
  return value;
}
...
function exactSubjects(context: EffectInvocationContext, eventId: EventId): boolean {
  return context.scope.eventId === eventId
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === INTAKE_FORM_CHANGESET_OWNER_ID
...
function operationPlan(input: {
...
}): FormChangesetPlan | undefined {
  if (input.operation.kind !== FORM_CHANGESET_KIND
      || input.operation.version !== FORM_CHANGESET_VERSION) return undefined;
  ...
  try {
    const plan = schema.schema.parse(input.operation.plan) as FormChangesetPlan;
    return canonicalJsonText(plan.policy) === canonicalJsonText(input.bundle.policy)
      ? plan
      : undefined;
  } catch {
    return undefined;
...
function currentFormApprovalPolicy(input: {
  readonly bundle: FormOrdinaryChangesetBundle;
  readonly operations: readonly FrozenChangesetOperation[];
}) {
  const actions = new Set(input.operations.map((operation) => {
    const plan = formPlan({ bundle: input.bundle, operation });
    if (!plan) throw new TypeError('intake_form_changeset_owner_mismatch');
    return plan.mutation.action;
  }));
  if (actions.size !== 1) throw new TypeError('intake_form_changeset_mixed_actions');
  const action = actions.values().next().value;
  if (!action) throw new TypeError('intake_form_changeset_revision_missing');
  return captureFormOrdinaryApprovalPolicy({ policy: input.bundle.policy, action });
}

function ownsFormChangeset(input: {
  readonly bundle: FormOrdinaryChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
  ...
  for (const revision of input.record.revisions) {
    ...
    try {
      if (canonicalJsonText(revision.approvalPolicy) !== canonicalJsonText(
        currentFormApprovalPolicy({ bundle: input.bundle, operations: revision.revision.operations })
      )) return false;
    } catch {
      return false;
    }
...
export class SQLiteIntakeFormChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle: FormOrdinaryChangesetBundle;
  readonly #ids: SQLiteIntakeFormChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    for (const method of [
      ...
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('intake_form_changeset_id_factory_invalid');
      }
    }
    ...
    ) as unknown as SQLiteIntakeFormChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:${INTAKE_FORM_CHANGESET_OWNER_ID}:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsFormChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: INTAKE_FORM_CHANGESET_OWNER_ID,
      evidenceIds: Object.freeze([
        `intake-form-definition:${this.#bundle.registry.registryDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) throw new TypeError('intake_form_changeset_transaction_required');
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('intake_form_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') throw new TypeError('intake_form_changeset_approval_not_mounted');
    ...
    if (action === undefined
      ...
      throw new TypeError('intake_form_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        )) throw new TypeError('intake_form_changeset_authority_mismatch');
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: INTAKE_FORM_CHANGESET_OWNER_ID
      },
      evaluatedAt
    });
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('intake_form_changeset_relationship_mismatch');
    }
    this.#prepared.clear();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('intake_form_changeset_context_substitution');
          }
          return this.prepare({
            action,
            businessInput,
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('intake_form_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('intake_form_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('intake_form_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
    ...
    if (!revision) throw new TypeError('intake_form_changeset_revision_missing');
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('intake_form_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('intake_form_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('intake_form_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('intake_form_changeset_commit_missing');
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('intake_form_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('intake_form_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO intake_form_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO intake_form_changeset_outbox_pointers (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('intake_form_changeset_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
  ...
  private nextId(method: keyof SQLiteIntakeFormChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('intake_form_changeset_ids_not_unique');
    ...
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        ...
        if ((key as unknown) === formPlanningAttributionReadPort) {
          return Object.freeze({
            readFormPlanningAttribution(scope: {
        ...
        throw new TypeError('intake_form_changeset_undeclared_read_port');
      }
    });
  }
    ...
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        ...
        if ((key as unknown) !== formChangesetValidationPort
            ...
            && (key as unknown) !== formCloseDeadlineTransactionPort) {
          throw new TypeError('intake_form_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('intake_form_changeset_event_required');
    const scope = {
      workspaceId: parseWorkspaceId(input.record.head.workspaceId),
      eventId: parseEventId(eventId)
    };
    const catalog = input.repository.readFormCatalog(scope);
    if (!catalog) throw new TypeError('intake_form_changeset_domain_missing');
    const aggregateVersions = new Map<string, number>();
    for (const head of catalog.heads) {
      aggregateVersions.set(formAggregateId(head.id), head.version);
    }
    const guardVersions = new Map<string, number>([
      [formCatalogGuardId(eventId), catalog.version]
    ]);
    const guardDigests = new Map<string, string>([
      [formCatalogGuardId(eventId), formCatalogDigest(catalog)]
    ]);
    const registry = input.repository.readFieldRegistrySnapshot(scope);
    if (registry) {
      aggregateVersions.set(`field_registry:${eventId}`, registry.version);
      guardVersions.set(`field_registry_guard:${eventId}`, registry.version);
      guardDigests.set(`field_registry_guard:${eventId}`, registry.registryDigestSha256);
    }
    const deadlineCatalog = input.repository.readDeadlineCatalog(scope);
    if (deadlineCatalog) {
    ...
    const eventTimeBasis = input.repository.readDeadlineEventTimeBasis(scope);
    if (eventTimeBasis) aggregateVersions.set(`event:${eventId}`, eventTimeBasis.eventVersion);
    const head = input.record.revisions.at(-1);
    if (!head) throw new TypeError('intake_form_changeset_revision_missing');
    for (const operation of head.revision.operations) {
      const plan = formPlan({ bundle: this.#bundle, operation });
      if (!plan) throw new TypeError('intake_form_changeset_owner_mismatch');
      const targetPin = plan.mutation.targetPin;
      if (targetPin?.kind === 'category') {
        const current = input.repository.resolveActiveCategory(scope, {
          kind: 'category',
          category: { kind: targetPin.categoryKind, id: targetPin.id }
        });
        if (current) {
          aggregateVersions.set(`program_${current.categoryKind}:${current.id}`, current.version);
        }
      } else if (targetPin?.kind === 'session') {
        const current = input.repository.resolveCollectingSession(scope, {
          kind: 'session', sessionId: targetPin.id
        });
        if (current) aggregateVersions.set(`session:${current.id}`, current.version);
      }
      for (const ref of operation.aggregateRefs) {
        if (!ref.id.startsWith('deadline:')) continue;
        const current = input.repository.readDeadline(scope, ref.id.slice('deadline:'.length));
        if (current) aggregateVersions.set(ref.id, current.version);
      }
      if (plan.mutation.action === 'publish'
          || (plan.mutation.action === 'lifecycle'
            && plan.mutation.publishedVersion !== null)) {
        const versions = input.repository.readFormVersions(scope, plan.mutation.before.id);
  ...
  private approvalPolicy(record: StoredChangesetRecord, revisionId: string) {
    ...
    if (!revision || revision.revision.operations.length === 0) {
      throw new TypeError('intake_form_changeset_revision_missing');
    }
    return currentFormApprovalPolicy({
      bundle: this.#bundle,
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === INTAKE_FORM_CHANGESET_OWNER_ID
      ? record
      : undefined;
  }
}

export function createSQLiteIntakeFormChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly policy: FormOrdinaryPolicy;
  readonly repository: SQLiteIntakeRepository;
  ...
  const adapter = new SQLiteIntakeFormChangesetEffectDomainAdapter(input);
  return Object.freeze({
    ownerId: INTAKE_FORM_CHANGESET_OWNER_ID,
