...
describe('SQLite epoch-2 retained-baseline runner', () => {
  ...
  test('upgrades retained profile approvals without inventing a user actor', () => {
    ...
    expectFoundationError(() => migrateOrValidateSQLite({
      ...
      isMemory: false,
      fault(point) {
        if (point === 'after_schema_before_receipt' && (schemaPass += 1) === 15) {
          throw new Error('hold_before_e2_0015_receipt');
        }
      }
  ...
  test('preserves issued API keys and immutable scopes when expiry becomes nullable', () => {
    ...
    expect(migrated.migration).toMatchObject({
      status: 'applied', migrationId: 'e2_0015_speaker_profile_review_policy',
      coordinate: { schemaEpoch: 2, sequence: 15 }
    });
