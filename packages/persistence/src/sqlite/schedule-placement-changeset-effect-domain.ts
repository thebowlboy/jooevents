...
import {
  ...
  schedulePlacementRoomQueryGuard,
...
export const SCHEDULE_PLACEMENT_CHANGESET_EFFECT_SQL = `
CREATE TABLE schedule_placement_changeset_receipt_links (
...
CREATE TABLE schedule_placement_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'schedule_occurrence_changed'),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES schedule_placement_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE schedule_placement_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES schedule_placement_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE schedule_placement_changeset_timeline (
...
CREATE TRIGGER schedule_placement_changeset_receipt_links_no_update
BEFORE UPDATE ON schedule_placement_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'schedule changeset receipt links are immutable'); END;
CREATE TRIGGER schedule_placement_changeset_receipt_links_no_delete
BEFORE DELETE ON schedule_placement_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'schedule changeset receipt links are immutable'); END;
CREATE TRIGGER schedule_placement_changeset_domain_facts_no_update
BEFORE UPDATE ON schedule_placement_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'schedule changeset facts are immutable'); END;
CREATE TRIGGER schedule_placement_changeset_domain_facts_no_delete
BEFORE DELETE ON schedule_placement_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'schedule changeset facts are immutable'); END;
CREATE TRIGGER schedule_placement_changeset_outbox_pointers_no_update
BEFORE UPDATE ON schedule_placement_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'schedule changeset pointers are immutable'); END;
CREATE TRIGGER schedule_placement_changeset_outbox_pointers_no_delete
BEFORE DELETE ON schedule_placement_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'schedule changeset pointers are immutable'); END;
CREATE TRIGGER schedule_placement_changeset_timeline_no_update
BEFORE UPDATE ON schedule_placement_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'schedule changeset timeline is immutable'); END;
CREATE TRIGGER schedule_placement_changeset_timeline_no_delete
BEFORE DELETE ON schedule_placement_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'schedule changeset timeline is immutable'); END;
`;

export function installSchedulePlacementChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('schedule_changeset_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(SCHEDULE_PLACEMENT_CHANGESET_EFFECT_SQL)).immediate();
}
...
export interface SQLiteSchedulePlacementChangesetEffectIds extends ChangesetLifecycleIds {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`schedule_changeset_${label}_invalid`);
  }
  return value;
}
...
function exactSubjects(context: EffectInvocationContext, eventId: EventId): boolean {
  return context.scope.eventId === eventId
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === 'schedule_placement'
    );
}

function operationPlan(input: {
  readonly bundle: SchedulePlacementChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): SchedulePlacementPlanDto | undefined {
  if (input.operation.kind !== SCHEDULE_PLACEMENT_CHANGESET_KIND
      || input.operation.version !== SCHEDULE_PLACEMENT_CHANGESET_VERSION) return undefined;
  ...
  return schedulePlacementPlanSchema.parse(input.operation.plan);
}

function schedulePlan(input: {
  readonly bundle: SchedulePlacementChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): SchedulePlacementPlanDto | undefined {
...
function ownsScheduleChangeset(input: {
  readonly bundle: SchedulePlacementChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
  ...
  for (const revision of input.record.revisions) {
    for (const operation of revision.revision.operations) {
      const plan = schedulePlan({ bundle: input.bundle, operation });
      if (!plan
          || plan.input.scope.workspaceId !== input.record.head.workspaceId
...
export class SQLiteSchedulePlacementChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #bundle = createSchedulePlacementChangesetBundle();
  ...
  constructor(private readonly input: {
    ...
    for (const method of [
      ...
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('schedule_changeset_id_factory_invalid');
      }
    }
    ...
    ) as unknown as SQLiteSchedulePlacementChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:schedule_placement:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsScheduleChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: 'schedule_placement',
      evidenceIds: Object.freeze([
        `schedule-placement-definition:${this.#bundle.registry.registryDigestSha256}`
      ])
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) throw new TypeError('schedule_changeset_transaction_required');
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('schedule_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') throw new TypeError('schedule_changeset_approval_not_mounted');
    ...
    if (action === undefined
      ...
      throw new TypeError('schedule_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        )) throw new TypeError('schedule_changeset_authority_mismatch');
    ...
    const subjectRelationship = this.subjectRelationships.validateSubject({
      ...
      userId: actorUserId,
      subject: {
        kind: 'domain', domain: 'changeset', entity: 'owner', id: 'schedule_placement'
      },
      evaluatedAt
    });
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('schedule_changeset_relationship_mismatch');
    }
    this.#prepared.clear();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('schedule_changeset_context_substitution');
          }
  ...
  private prepare(input: {
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('schedule_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('schedule_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('schedule_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
      ...
      const validation = validateStoredChangesetCommit({
        ...
        context: actorContext,
        ...wire,
        currentApprovalPolicy: SCHEDULE_PLACEMENT_APPROVAL_POLICY,
    ...
    if (!revision) throw new TypeError('schedule_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: 'schedule_occurrence_changed',
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('schedule_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('schedule_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('schedule_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('schedule_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO schedule_placement_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('schedule_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('schedule_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO schedule_placement_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO schedule_placement_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO schedule_placement_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('schedule_changeset_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
  ...
  private nextId(method: keyof SQLiteSchedulePlacementChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('schedule_changeset_ids_not_unique');
  ...
  private planningSnapshot(repository: SQLiteSchedulePlacementRepository): ChangesetPlanningSnapshot {
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== schedulePlacementReadPort) {
          throw new TypeError('schedule_changeset_undeclared_read_port');
        }
  ...
  private commitTransaction(repository: SQLiteSchedulePlacementRepository): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        if ((key as unknown) !== schedulePlacementValidationPort
            && (key as unknown) !== schedulePlacementTransactionPort) {
          throw new TypeError('schedule_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  private currentCommitEvidence(input: {
    readonly record: StoredChangesetRecord;
    readonly repository: SchedulePlacementTransactionPort;
  }) {
    const eventId = input.record.head.eventId;
    if (eventId === undefined) throw new TypeError('schedule_changeset_event_required');
    const scope = {
      workspaceId: parseWorkspaceId(input.record.head.workspaceId),
      eventId: parseEventId(eventId)
    };
    const state = input.repository.readSchedule(scope);
    const vocabulary = input.repository.readVocabulary(scope);
    if (!state || !vocabulary) throw new TypeError('schedule_changeset_domain_missing');
    const aggregateVersions = new Map<string, number>();
    for (const occurrence of state.occurrences) {
      aggregateVersions.set(`schedule_occurrence:${occurrence.id}`, occurrence.version);
    }
    ...
    const guardVersions = new Map<string, number>([
      [schedulePlacementGuardId(eventId), state.scheduleVersion],
      [programVocabularySetGuardId(eventId), vocabulary.setVersion]
    ]);
    const guardDigests = new Map<string, string>([
      [schedulePlacementGuardId(eventId), schedulePlacementStateDigest(state)],
      [programVocabularySetGuardId(eventId), programVocabularySetDigest(vocabulary)]
    ]);
    const head = input.record.revisions.at(-1);
    if (!head) throw new TypeError('schedule_changeset_revision_missing');
    for (const operation of head.revision.operations) {
      const plan = schedulePlan({ bundle: this.#bundle, operation });
      if (!plan) throw new TypeError('schedule_changeset_owner_mismatch');
      const roomId = plan.after?.roomId ?? plan.before?.roomId;
      if (!roomId) throw new TypeError('schedule_changeset_room_missing');
      const query = schedulePlacementRoomQueryGuard(
        state,
        roomId,
        plan.before?.id
      );
      guardVersions.set(query.id, query.version);
      guardDigests.set(query.id, query.digestSha256);
    }
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === 'schedule_placement'
      ? record
      : undefined;
  }
}
...
export function createSQLiteSchedulePlacementChangesetEffectDomainRegistration(input: {
  ...
  const adapter = new SQLiteSchedulePlacementChangesetEffectDomainAdapter(input);
  return Object.freeze({
