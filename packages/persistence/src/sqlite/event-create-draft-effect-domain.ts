...
import {
  EVENT_CREATE_DRAFT_HANDLER_CAPABILITY,
  EVENT_CREATE_DRAFT_OPERATION,
  EVENT_MANAGE_ACCESS_POLICY,
  eventCreateDraftContributionSchema,
  eventCreateDraftDomainContributionSchema,
  eventCreateDraftEvidenceChildSchema,
  sealEventCreateDraftPreparation,
  type EventCreateDraftContribution
...
export const EVENT_CREATE_DRAFT_EFFECT_SQL = `
CREATE TABLE event_create_draft_receipt_links (
  ...
  operation_name TEXT NOT NULL CHECK(operation_name = 'event.create.draft'),
  ...
  FOREIGN KEY(changeset_id)
    REFERENCES changeset_heads(changeset_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
...
CREATE TABLE event_create_draft_timeline (
  ...
  FOREIGN KEY(receipt_id, workspace_id, changeset_id, revision_id)
    REFERENCES event_create_draft_receipt_links(
      receipt_id, workspace_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TRIGGER event_create_draft_receipt_links_no_update
BEFORE UPDATE ON event_create_draft_receipt_links
BEGIN SELECT RAISE(ABORT, 'event create draft receipt links are immutable'); END;
CREATE TRIGGER event_create_draft_receipt_links_no_delete
BEFORE DELETE ON event_create_draft_receipt_links
BEGIN SELECT RAISE(ABORT, 'event create draft receipt links are immutable'); END;
CREATE TRIGGER event_create_draft_timeline_no_update
BEFORE UPDATE ON event_create_draft_timeline
BEGIN SELECT RAISE(ABORT, 'event create draft timeline is immutable'); END;
CREATE TRIGGER event_create_draft_timeline_no_delete
BEFORE DELETE ON event_create_draft_timeline
BEGIN SELECT RAISE(ABORT, 'event create draft timeline is immutable'); END;
`;

export function installEventCreateDraftEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) throw new TypeError('event_create_draft_schema_inside_transaction');
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(EVENT_CREATE_DRAFT_EFFECT_SQL)).immediate();
}

export interface SQLiteEventCreateDraftEffectIds {
...
function applicationUuid(value: unknown, label: string): string {
  if (typeof value !== 'string' || !APPLICATION_UUID.test(value)) {
    throw new TypeError(`event_create_draft_${label}_invalid`);
  }
  return value.toLowerCase();
}

function exactCapability(value: { readonly key: string; readonly version: number }): boolean {
  return value.key === EVENT_CREATE_DRAFT_HANDLER_CAPABILITY.key
    && value.version === EVENT_CREATE_DRAFT_HANDLER_CAPABILITY.version;
}
...
function collisionContribution(): EventCreateDraftContribution {
  return eventCreateDraftContributionSchema.parse({
    result: {
      kind: 'outcome',
      outcome: {
        class: 'conflict',
...
function planningRefusal(
  error: EventPlanningError,
  eventId: string
): EventCreateDraftContribution | undefined {
  if (error.code !== 'stale_event_set' && error.code !== 'event_already_selected') {
    return undefined;
  }
  return eventCreateDraftContributionSchema.parse({
    result: {
      kind: 'outcome',
      outcome: {
        class: error.code === 'stale_event_set' ? 'stale_revision' : 'conflict',
        kind: error.code === 'stale_event_set'
          ? 'event.creation_changed'
          : 'event.already_selected',
        retryable: false,
        subjects: [{ type: 'event', id: eventId }],
        detail: { code: error.code, action: 'create', eventId },
        detailSchemaVersion: 1
      }
    },
    domain: null,
...
/** Persists an inert workspace-scoped Event creation draft in the Foundation UoW. */
export class SQLiteEventCreateDraftEffectDomainAdapter implements SQLiteEffectDomainAdapter {
  readonly #bundle: EventCreationOrdinaryChangesetBundle;
  readonly #changesets: SQLiteChangesetLifecycleStore;
  readonly #ids: SQLiteEventCreateDraftEffectIds;
  ...
  constructor(private readonly input: {
    ...
    for (const method of [
      ...
      'newTimelineId', 'newEventId'
    ] as const) {
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('event_create_draft_id_factory_invalid');
      }
    }
    this.#ids = Object.freeze(Object.fromEntries(
      (['newChangesetId', 'newRevisionId', 'newPreparationHandle',
        'newTimelineId', 'newEventId'] as const)
        .map((method) => [method, input.ids[method].bind(input.ids)])
    ) as unknown as SQLiteEventCreateDraftEffectIds);
  }

  private nextId(method: keyof SQLiteEventCreateDraftEffectIds): string {
    const value = applicationUuid(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('event_create_draft_ids_not_unique');
    this.#issuedIds.add(value);
    return value;
  }

  private readPort(): EventCreationReadPort {
    const repository = new SQLiteEventSpineRepository(this.input.sqlite);
    return createEventCreationValidationView(Object.freeze({
      readEventSet: (workspaceId: string) => repository.readEventSet(workspaceId),
      readEvent: (workspaceId: string, eventId: string) =>
        repository.readEvent(workspaceId, eventId),
      readContributor: (
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) {
      throw new TypeError('event_create_draft_transaction_required');
    }
    if (!exactCapability(capability)) {
      throw new TypeError('event_create_draft_capability_mismatch');
    }
    if (context.operation.name !== EVENT_CREATE_DRAFT_OPERATION.name
        || context.operation.version !== EVENT_CREATE_DRAFT_OPERATION.version
      ...
      throw new TypeError('event_create_draft_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        )) throw new TypeError('event_create_draft_authority_mismatch');
    ...
    const snapshot: ChangesetPlanningSnapshot = Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== eventCreationReadPort) {
          throw new TypeError('event_create_draft_undeclared_read_port');
        }
    ...
    return sealEventCreateDraftPreparation({
      capability,
      context,
      preparation: {
        prepare: ({ businessInput, context: receivedContext }) => {
          if (receivedContext !== context || !this.input.sqlite.inTransaction) {
            throw new TypeError('event_create_draft_context_substitution');
          }
          const business = eventCreateDraftInputSchema.parse(businessInput);
          ...
          const eventId = parseEventId(this.nextId('newEventId'));
          const before = new SQLiteEventSpineRepository(this.input.sqlite)
            .readCurrentEventState(this.input.workspaceId);
          if (!before) throw new TypeError('event_create_draft_workspace_missing');
          const approvalPolicy = captureEventOrdinaryApprovalPolicy({
          ...
          try {
            appended = appendChangesetDraftSynchronous({
              ...
              dependencyGroups: [{ key: 'event_creation', dependsOn: [] }],
              approvalPolicy,
              origin: 'human_ui'
            });
          } catch (error) {
            if (error instanceof EventPlanningError) {
              const refusal = planningRefusal(error, eventId);
              if (refusal) {
          ...
          if (appended.kind === 'refused') {
            if (appended.refusal.kind !== 'id_collision') {
              throw new TypeError('event_create_draft_unexpected_lifecycle_refusal');
            }
            this.#nonterminalReleaseContext = context;
            return collisionContribution();
          }
          const after = new SQLiteEventSpineRepository(this.input.sqlite)
            .readCurrentEventState(this.input.workspaceId);
          if (!after || canonicalJsonText({
            eventSet: after.eventSet,
            currentEvent: after.currentEvent ?? null
          }) !== canonicalJsonText({
            eventSet: before.eventSet,
            currentEvent: before.currentEvent ?? null
          })) {
            throw new TypeError('event_create_draft_mutated_effective_state');
          }
          ...
          if (!revision || !operation
              ...
              || appended.record.head.eventId !== undefined) {
            throw new TypeError('event_create_draft_record_incoherent');
          }
          const safeDiff = eventCreateSafeDiffSchema.parse(operation.safeDiff);
          const candidate = eventCreateDraftContributionSchema.parse({
            result: {
              kind: 'success',
              data: {
                action: 'create',
            ...
            domain: {
              kind: 'event_creation_changeset_draft',
          ...
          if (candidate.result.kind !== 'success' || candidate.domain === null) {
            throw new TypeError('event_create_draft_success_contribution_invalid');
          }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    const parsed = eventCreateDraftDomainContributionSchema.parse(contribution);
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    const parsedResult = eventCreateDraftOperationResultSchema.safeParse(receipt.result);
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
        ...
        || receipt.ref.operationName !== EVENT_CREATE_DRAFT_OPERATION.name
        || receipt.ref.operationVersion !== EVENT_CREATE_DRAFT_OPERATION.version
        || !parsedResult.success || parsedResult.data.kind !== 'success'
        || parsedResult.data.receipt.id !== receipt.ref.id
        || parsedResult.data.receipt.operationName !== EVENT_CREATE_DRAFT_OPERATION.name
        || parsedResult.data.receipt.operationVersion !== EVENT_CREATE_DRAFT_OPERATION.version
        || canonicalJsonText(parsedResult.data.data)
          !== canonicalJsonText(active.contribution.result.data)) {
      throw new TypeError('event_create_draft_receipt_mismatch');
    }
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO event_create_draft_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('event_create_draft_receipt_parent_missing');
    }
    const child = eventCreateDraftEvidenceChildSchema.parse(contribution);
    const expected = active.contribution.receiptChildren[0];
    if (canonicalJsonText(child) !== canonicalJsonText(expected)) {
      throw new TypeError('event_create_draft_evidence_mismatch');
    }
    this.input.sqlite.query<never, [string, string, string, string, string, string, number, string]>(`
      INSERT INTO event_create_draft_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('event_create_draft_incomplete');
      }
      this.#nonterminalReleaseContext = undefined;
      return;
    }
...
export function createSQLiteEventCreateDraftEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly policy: EventOrdinaryPolicy;
  readonly dependencyRegistry: EventDependencyContributorRegistry;
  ...
  readonly capability: typeof EVENT_CREATE_DRAFT_HANDLER_CAPABILITY;
  readonly adapter: SQLiteEventCreateDraftEffectDomainAdapter;
} {
  return Object.freeze({
    capability: EVENT_CREATE_DRAFT_HANDLER_CAPABILITY,
    adapter: new SQLiteEventCreateDraftEffectDomainAdapter(input)
  });
}
