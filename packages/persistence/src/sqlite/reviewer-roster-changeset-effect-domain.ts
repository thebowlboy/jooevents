...
import { REVIEWER_ROSTER_CHANGE_APPROVAL_POLICY } from './reviewer-roster-draft-effect-domain';

/** Stable changeset-owner identity for every reviewer-roster changeset. */
export const REVIEWER_ROSTER_CHANGESET_OWNER_ID = 'reviewer_roster' as const;

export const REVIEWER_ROSTER_CHANGESET_EFFECT_SQL = `
CREATE TABLE reviewer_roster_changeset_receipt_links (
...
CREATE TABLE reviewer_roster_changeset_domain_facts (
  ...
  changeset_id TEXT NOT NULL,
  revision_id TEXT NOT NULL,
  fact_kind TEXT NOT NULL CHECK(fact_kind = 'reviewer_roster_changed'),
  ...
  FOREIGN KEY(receipt_id, workspace_id, event_id, changeset_id, revision_id)
    REFERENCES reviewer_roster_changeset_receipt_links(
      receipt_id, workspace_id, event_id, changeset_id, revision_id
    ) ON UPDATE RESTRICT ON DELETE RESTRICT,
  UNIQUE(fact_id, receipt_id)
) STRICT, WITHOUT ROWID;

CREATE TABLE reviewer_roster_changeset_outbox_pointers (
  pointer_id TEXT PRIMARY KEY,
  receipt_id TEXT NOT NULL UNIQUE,
  fact_id TEXT NOT NULL UNIQUE,
  source_kind TEXT NOT NULL CHECK(source_kind = 'domain_fact'),
  FOREIGN KEY(fact_id, receipt_id)
    REFERENCES reviewer_roster_changeset_domain_facts(fact_id, receipt_id)
    ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT, WITHOUT ROWID;

CREATE TABLE reviewer_roster_changeset_timeline (
...
CREATE TRIGGER reviewer_roster_changeset_receipt_links_no_update
BEFORE UPDATE ON reviewer_roster_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset receipt links are immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_receipt_links_no_delete
BEFORE DELETE ON reviewer_roster_changeset_receipt_links
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset receipt links are immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_domain_facts_no_update
BEFORE UPDATE ON reviewer_roster_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset facts are immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_domain_facts_no_delete
BEFORE DELETE ON reviewer_roster_changeset_domain_facts
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset facts are immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_outbox_pointers_no_update
BEFORE UPDATE ON reviewer_roster_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset pointers are immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_outbox_pointers_no_delete
BEFORE DELETE ON reviewer_roster_changeset_outbox_pointers
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset pointers are immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_timeline_no_update
BEFORE UPDATE ON reviewer_roster_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset timeline is immutable'); END;
CREATE TRIGGER reviewer_roster_changeset_timeline_no_delete
BEFORE DELETE ON reviewer_roster_changeset_timeline
BEGIN SELECT RAISE(ABORT, 'reviewer roster changeset timeline is immutable'); END;
`;

export function installReviewerRosterChangesetEffectSchema(sqlite: Database): void {
  if (sqlite.inTransaction) {
    throw new TypeError('reviewer_roster_changeset_schema_inside_transaction');
  }
  sqlite.exec('PRAGMA foreign_keys = ON');
  sqlite.transaction(() => sqlite.exec(REVIEWER_ROSTER_CHANGESET_EFFECT_SQL)).immediate();
}

export interface SQLiteReviewerRosterChangesetEffectIds extends ChangesetLifecycleIds {
...
function applicationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !isApplicationId(value)) {
    throw new TypeError(`reviewer_roster_changeset_${label}_invalid`);
  }
  return value;
}
...
function exactSubjects(context: EffectInvocationContext, eventId: EventId): boolean {
  return context.scope.eventId === eventId
    ...
    && context.scope.subjects.some((subject) =>
      ...
      && subject.id === REVIEWER_ROSTER_CHANGESET_OWNER_ID
    );
}

function operationPlan(input: {
  readonly bundle: ReviewerRosterChangesetBundle;
  readonly operation: FrozenChangesetOperation;
}): ReviewerRosterMutationPlanDto | undefined {
  if (input.operation.kind !== REVIEWER_ROSTER_CHANGESET_KIND
      || input.operation.version !== REVIEWER_ROSTER_CHANGESET_VERSION) return undefined;
  ...
  return schema?.schema.parse(input.operation.plan) as ReviewerRosterMutationPlanDto | undefined;
}

function ownsReviewerRosterChangeset(input: {
  readonly bundle: ReviewerRosterChangesetBundle;
  readonly record: StoredChangesetRecord;
}): boolean {
...
/**
 * Owns the reviewer-roster changeset transitions and the effective roster commit
 * on one caller-owned SQLite handle. The correction path constructs the roster
 * repository with a fresh compensation-attribution callback bound to the acting
 * operator, so an exact compensating changeset is derivable instead of blocking
 * with `reviewer_roster.fresh_attribution_required`.
 */
export class SQLiteReviewerRosterChangesetEffectDomainAdapter
implements SQLiteEffectDomainAdapter, ChangesetLifecycleOwnerResolutionSource {
  ...
  readonly #ids: SQLiteReviewerRosterChangesetEffectIds;
  ...
  constructor(private readonly input: {
    ...
    for (const method of ID_METHODS) {
      if (typeof input.ids[method] !== 'function') {
        throw new TypeError('reviewer_roster_changeset_id_factory_invalid');
      }
    }
    this.#ids = Object.freeze(Object.fromEntries(
      ID_METHODS.map((method) => [method, input.ids[method].bind(input.ids)])
    ) as unknown as SQLiteReviewerRosterChangesetEffectIds);
        ...
        return rows.length === 1 && rows[0]?.event_id === eventId
          ? Object.freeze({
              ...
              evidenceIds: Object.freeze([
                `changeset-owner:reviewer_roster:${eventId}:${this.#bundle.registry.registryDigestSha256}`
              ])
            })
  ...
  resolveOwner(record: StoredChangesetRecord): ChangesetLifecycleOwnerResolution | undefined {
    if (!ownsReviewerRosterChangeset({ bundle: this.#bundle, record })) return undefined;
    return Object.freeze({
      id: REVIEWER_ROSTER_CHANGESET_OWNER_ID,
      evidenceIds: Object.freeze([
        `reviewer-roster-definition:${this.#bundle.registry.registryDigestSha256}`
      ]),
      diffReadPermissionIds: Object.freeze(
        [...REVIEWER_ROSTER_PERMISSION_IDS]
      ) as readonly [PermissionId, ...PermissionId[]]
    });
  }
  ...
  openHandlerSnapshot(
    ...
    if (!this.input.sqlite.inTransaction) {
      throw new TypeError('reviewer_roster_changeset_transaction_required');
    }
    if (!sameReference(capability, CHANGESET_LIFECYCLE_HANDLER_CAPABILITY)) {
      throw new TypeError('reviewer_roster_changeset_capability_mismatch');
    }
    const rawAction = changesetLifecycleActionForOperation(
      context.operation.name, context.operation.version
    );
    if (rawAction === 'approve') {
      throw new TypeError('reviewer_roster_changeset_approval_not_mounted');
    }
    ...
    if (action === undefined
      ...
      throw new TypeError('reviewer_roster_changeset_scope_mismatch');
    }
    ...
    if (authority.actor.kind !== 'workspace_user'
        ...
        || !REVIEWER_ROSTER_PERMISSION_IDS.every((permissionId) =>
          ...
          ))) throw new TypeError('reviewer_roster_changeset_authority_mismatch');
    ...
    if (relationship.kind !== 'valid' || subjectRelationship.kind !== 'valid') {
      throw new TypeError('reviewer_roster_changeset_relationship_mismatch');
    }
    this.clearTransient();
      ...
      preparation: Object.freeze({
        prepare: ({ action: receivedAction, businessInput, context: receivedContext }:
          ...
          if (receivedAction !== action || receivedContext !== context
            ...
            throw new TypeError('reviewer_roster_changeset_context_substitution');
          }
  ...
  private prepare(input: {
    ...
    const repository = this.repository(input.eventId, input.actorUserId, input.evaluatedAt);
    ...
    if (input.action === 'propose') {
      ...
      if (!diff) throw new TypeError('reviewer_roster_changeset_proposal_diff_missing');
      ...
      if (!diff) throw new TypeError('reviewer_roster_changeset_rebuild_diff_missing');
      data = {
        schemaVersion: 1,
        action: 'rebuild',
        sourceRevisionId: wire.sourceRevisionId,
      ...
      if (result.record !== null && !target) {
        throw new TypeError('reviewer_roster_changeset_correction_diff_missing');
      }
      data = {
        schemaVersion: 1,
        action: 'correction',
      ...
      const validation = validateStoredChangesetCommit({
        ...
        context: actorContext,
        ...wire,
        currentApprovalPolicy: REVIEWER_ROSTER_CHANGE_APPROVAL_POLICY,
      ...
      if (facts.length !== 1
          || facts[0]?.kind !== 'reviewer_roster_changed'
          || facts[0].version !== 1) {
        throw new TypeError('reviewer_roster_changeset_fact_contribution_invalid');
      }
    ...
    if (!revision) throw new TypeError('reviewer_roster_changeset_revision_missing');
    ...
    const receiptChildren = input.action === 'commit'
      ? [{
          ...
          factKind: 'reviewer_roster_changed',
          factVersion: 1,
          workspaceId: this.input.workspaceId,
          eventId: input.eventId,
          changesetId: record.head.id,
    ...
    if (candidate.result.kind !== 'success' || candidate.domain === null) {
      throw new TypeError('reviewer_roster_changeset_success_contribution_invalid');
    }
  ...
  applyDomainContribution(contribution: unknown): void {
    ...
    if (!prepared || prepared.phase !== 'prepared'
      ...
      throw new TypeError('reviewer_roster_changeset_preparation_invalid');
    }
  ...
  afterReceiptParentInserted(receipt: TerminalEffectReceipt): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'applied'
      ...
      throw new TypeError('reviewer_roster_changeset_receipt_mismatch');
    }
    const receiptId = parseOperationReceiptId(receipt.ref.id);
    let record = active.record;
    if (active.action === 'commit') {
      if (!active.exactCommit) throw new TypeError('reviewer_roster_changeset_commit_missing');
    ...
    this.input.sqlite.query<never, [
      ...
      INSERT INTO reviewer_roster_changeset_receipt_links (
  ...
  afterReceiptChildInserted(receiptId: string, contribution: unknown): void {
    ...
    if (!this.input.sqlite.inTransaction || !active || active.phase !== 'parent_linked'
      ...
      throw new TypeError('reviewer_roster_changeset_receipt_parent_missing');
    }
    ...
    if (!expected || canonicalJsonText(contribution) !== canonicalJsonText(expected)) {
      throw new TypeError('reviewer_roster_changeset_evidence_mismatch');
    }
    if ((contribution as { readonly kind?: unknown }).kind === 'domain_fact') {
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO reviewer_roster_changeset_domain_facts (
      ...
      this.input.sqlite.query<never, [string, string, string, string]>(`
        INSERT INTO reviewer_roster_changeset_outbox_pointers (
      ...
      this.input.sqlite.query<never, [
        ...
        INSERT INTO reviewer_roster_changeset_timeline (
  ...
  afterExecutionClaimReleased(identity: EffectOperationIdentity): void {
    ...
    if (!active) {
      ...
      if (!context || !effectOperationIdentityMatchesContext(identity, context)) {
        throw new TypeError('reviewer_roster_changeset_incomplete');
      }
  ...
  private nextId(method: keyof SQLiteReviewerRosterChangesetEffectIds): string {
    const value = applicationId(this.#ids[method](), method);
    if (this.#issuedIds.has(value)) throw new TypeError('reviewer_roster_changeset_ids_not_unique');
    this.#issuedIds.add(value);
    return value;
  }

  /**
   * The composed roster repository for one prepared lifecycle step. Compensation
   * attribution is bound to the acting operator at the current recheck time, so
   * a derived compensation carries fresh attribution instead of blocking.
   */
  private repository(
    eventId: EventId,
    actorUserId: UserId,
    ...
    return new SQLiteReviewerRosterRepository(
      this.input.sqlite,
      this.input.sources,
      (scope: ReviewerRosterScopeDto) =>
        scope.workspaceId === this.input.workspaceId && scope.eventId === eventId
          ? { userId: actorUserId, occurredAt: evaluatedAt }
          : undefined
    );
  }

  private planningSnapshot(repository: SQLiteReviewerRosterRepository): ChangesetPlanningSnapshot {
    return Object.freeze({
      getPort<Port>(key: ChangesetReadPortKey<Port>): Port {
        if ((key as unknown) !== reviewerRosterChangesetReadPort) {
          throw new TypeError('reviewer_roster_changeset_undeclared_read_port');
        }
  ...
  private commitTransaction(repository: SQLiteReviewerRosterRepository): ChangesetCommitTransaction {
    return Object.freeze({
      getPort<Port>(key: ChangesetValidationPortKey<Port> | ChangesetTransactionPortKey<Port>): Port {
        if ((key as unknown) !== reviewerRosterChangesetValidationPort
            && (key as unknown) !== reviewerRosterChangesetTransactionPort) {
          throw new TypeError('reviewer_roster_changeset_undeclared_transaction_port');
        }
        return repository as unknown as Port;
      }
    });
  ...
  private currentCommitEvidence(input: {
    ...
    if (eventId === undefined) throw new TypeError('reviewer_roster_changeset_event_required');
    const scope: ReviewerRosterScopeDto = {
    ...
    const authority = repository.readReviewerAuthority(scope);
    const targets = repository.readReviewerScopeTargets(scope);
    if (!roster) throw new TypeError('reviewer_roster_changeset_domain_missing');
    ...
    aggregateVersions.set(reviewerRosterGuardId(eventId), roster.version);
    guardVersions.set(reviewerRosterGuardId(eventId), roster.version);
    guardDigests.set(reviewerRosterGuardId(eventId), roster.digestSha256);
    for (const reviewer of roster.reviewers) {
      aggregateVersions.set(`reviewer:${reviewer.reviewerId}`, reviewer.version);
    }
    if (authority) {
      guardVersions.set(`reviewer_authority_set:${eventId}`, authority.version);
      guardDigests.set(`reviewer_authority_set:${eventId}`, authority.digestSha256);
    }
    if (targets) {
      guardVersions.set(`reviewer_scope_targets:${eventId}`, targets.version);
      guardDigests.set(`reviewer_scope_targets:${eventId}`, targets.digestSha256);
      for (const target of targets.targets) {
        guardVersions.set(reviewerScopeTargetGuardId(target.ref), target.version);
        guardDigests.set(reviewerScopeTargetGuardId(target.ref), target.digestSha256);
      }
    }
    const head = input.record.revisions.at(-1);
    if (!head) throw new TypeError('reviewer_roster_changeset_revision_missing');
    for (const operation of head.revision.operations) {
      const plan = operationPlan({ bundle: this.#bundle, operation });
      if (!plan) throw new TypeError('reviewer_roster_changeset_owner_mismatch');
      if (!authority) continue;
      const fact = authority.facts.find((candidate) =>
        sameAuthoritySubject(candidate.rosterSubject, plan.after.accessSubject)
      );
      const guardId = `reviewer_authority:${plan.after.reviewerId}`;
      if (fact) {
        guardVersions.set(guardId, fact.version);
        guardDigests.set(guardId, fact.digestSha256);
      } else {
        const unsigned = {
          schemaVersion: 1 as const,
          scope: plan.after.scope,
          rosterSubject: plan.after.accessSubject,
          state: 'unavailable' as const,
          version: authority.version,
          capabilityIds: [] as const,
          evidenceIds: [`reviewer_authority_set:${authority.digestSha256}`]
        };
        guardVersions.set(guardId, authority.version);
        guardDigests.set(guardId, reviewerAuthorityFactDigest(unsigned as never));
      }
    }
  ...
  private exactRecord(changesetId: string, eventId: EventId): StoredChangesetRecord | undefined {
    ...
    return record
      ...
      && this.resolveOwner(record)?.id === REVIEWER_ROSTER_CHANGESET_OWNER_ID
      ? record
      : undefined;
  }
}

export function createSQLiteReviewerRosterChangesetEffectDomainRegistration(input: {
  readonly sqlite: Database;
  readonly workspaceId: WorkspaceId;
  readonly sources: ReviewerRosterPlanningSource;
  readonly eventRelationships: SQLiteOperatorEventRelationshipSource;
  ...
  const adapter = new SQLiteReviewerRosterChangesetEffectDomainAdapter(input);
  return Object.freeze({
    ownerId: REVIEWER_ROSTER_CHANGESET_OWNER_ID,
