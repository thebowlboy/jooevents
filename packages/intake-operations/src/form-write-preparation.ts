...
export function sealIntakeFormWritePreparation(
  input: Omit<State, 'phase'>
): EffectHandlerSnapshot {
  if (input.prepare.constructor.name === 'AsyncFunction') {
    throw new TypeError('intake_form_write_preparation_must_be_synchronous');
  }
  const snapshot = Object.freeze({ strategy: 'intake_form_owner_write', version: 1 });
  states.set(snapshot, { ...input, phase: 'ready' });
  return snapshot;
}

export function createIntakeFormWriteHandler(input: {
  ...
  return Object.freeze({
    ...
    effect: input.effect,
    handlerCapability: input.capability,
    ...
    handle({ businessInput, context, snapshot }: Parameters<EffectHandlerRegistration['handle']>[0]) {
      ...
      if (!state
          || state.capability.key !== input.capability.key
          || state.capability.version !== input.capability.version
          || state.context !== context
          || state.phase !== 'ready') {
        throw new TypeError('invalid_intake_form_write_preparation');
      }
      state.phase = 'spent';
