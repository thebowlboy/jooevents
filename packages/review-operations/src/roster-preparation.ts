...
export interface ReviewerRosterDraftPreparedContribution {
  readonly result: unknown;
  readonly domain: unknown;
  readonly receiptChildren: readonly unknown[];
}

/** Transaction-owned preparation for one inert reviewer-roster changeset draft. */
export interface ReviewerRosterDraftPreparation {
  prepare(input: {
    readonly businessInput: unknown;
    readonly context: EffectInvocationContext;
  }): ReviewerRosterDraftPreparedContribution;
}

interface SealedPreparation {
  readonly capability: VersionedDefinitionRef;
  readonly context: EffectInvocationContext;
  readonly prepare: ReviewerRosterDraftPreparation['prepare'];
  phase: 'ready' | 'preparing' | 'spent';
}

const sealedPreparations = new WeakMap<object, SealedPreparation>();

export function sealReviewerRosterDraftPreparation(input: {
  readonly capability: VersionedDefinitionRef;
  readonly context: EffectInvocationContext;
  readonly preparation: ReviewerRosterDraftPreparation;
}): EffectHandlerSnapshot {
  ...
  if (input.preparation.prepare.constructor.name === 'AsyncFunction') {
    throw new TypeError('reviewer_roster_draft_preparation_must_be_synchronous');
  }
  const snapshot = Object.freeze({ strategy: 'reviewer_roster_changeset_draft', version: 1 });
...
export function createReviewerRosterDraftHandler(input: {
  ...
  return Object.freeze({
    ...
    handle({ businessInput, context, snapshot }:
      ...
      if (!sealed
          ...
          || sealed.phase !== 'ready') {
        throw new TypeError('invalid_reviewer_roster_draft_preparation');
      }
