...
export interface SchedulePlacementDraftPreparedContribution {
  readonly result: unknown;
  readonly domain: unknown;
  readonly receiptChildren: readonly unknown[];
}

/** Transaction-owned preparation for one inert Schedule placement changeset draft. */
export interface SchedulePlacementDraftPreparation {
  prepare(input: {
    readonly businessInput: unknown;
    readonly context: EffectInvocationContext;
  }): SchedulePlacementDraftPreparedContribution;
}

interface SealedPreparation {
  readonly capability: VersionedDefinitionRef;
  readonly context: EffectInvocationContext;
  readonly prepare: SchedulePlacementDraftPreparation['prepare'];
...
export function sealSchedulePlacementDraftPreparation(input: {
  readonly capability: VersionedDefinitionRef;
  readonly context: EffectInvocationContext;
  readonly preparation: SchedulePlacementDraftPreparation;
}): EffectHandlerSnapshot {
  ...
  if (input.preparation.prepare.constructor.name === 'AsyncFunction') {
    throw new TypeError('schedule_placement_draft_preparation_must_be_synchronous');
  }
  const snapshot = Object.freeze({ strategy: 'schedule_placement_changeset_draft', version: 1 });
...
export function createSchedulePlacementDraftHandler(input: {
  ...
  return Object.freeze({
    ...
    handle({ businessInput, context, snapshot }: Parameters<EffectHandlerRegistration['handle']>[0]) {
      ...
      if (!sealed
          ...
          || sealed.phase !== 'ready') {
        throw new TypeError('invalid_schedule_placement_draft_preparation');
      }
