...
export function sealProgramVocabularyMergePreparation(
  input: Omit<State, 'phase'>
): EffectHandlerSnapshot {
  if (input.prepare.constructor.name === 'AsyncFunction') {
    throw new TypeError('program_vocabulary_merge_preparation_must_be_synchronous');
  }
  const snapshot = Object.freeze({ strategy: 'program_vocabulary_reviewed_merge', version: 1 });
  states.set(snapshot, { ...input, phase: 'ready' });
  return snapshot;
}

export function createProgramVocabularyMergeHandler(input: {
  ...
  return Object.freeze({
    ...
    handle({ businessInput, context, snapshot }: Parameters<EffectHandlerRegistration['handle']>[0]) {
      ...
      if (!state || state.capability.key !== input.capability.key
        ...
        throw new TypeError('invalid_program_vocabulary_merge_preparation');
      }
      state.phase = 'spent';
